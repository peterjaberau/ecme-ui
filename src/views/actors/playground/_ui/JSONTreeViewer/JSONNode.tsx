...
import SolidIcon from '@/views/actors/playground/_ui/Icon/SolidIcons'
...
export const JSONNode = ({ data, ...restProps }) => {
    ...
    const [pathToBeInspected, setPathToBeInspected] = React.useState(null)
    ...
    const [showHiddenOptionButtons, setShowHiddenOptionButtons] =
        React.useState([])

    React.useEffect(() => {
        if (showHiddenOptionButtons) {
            setShowHiddenOptionButtons(() =>
    ...
    const toExpandNode =
        (data instanceof Array || data instanceof Object) && !_.isEmpty(data)
    ...
    const checkSelectedNode = (
        ...
        if (selectedNode?.parent && parent) {
            return (
                _selectedNode.parent === parent &&
                _selectedNode?.node === _currentNode &&
                toExpand
            )
        }
    ...
    const parent =
        path && typeof path?.length === 'number' ? path[path.length - 2] : null
        ...
        case 'Object':
            ...
            $NODEType = (
                <JSONNode.DisplayNodeLabel type={'Object'}>
                    <span className="node-length-color">
    ...
    let $key = (
        <span
            onClick={() => {
                const shouldTriggerActions =
                    debuggerTree && currentNode === 'componentId'
            ...
            className={cx(
                'node-key badge badge-outline hovered-json-node font-semibold text-gray-900 dark:text-gray-100',
                {
    ...
    const renderHiddenOptionsForNode = () => {
        const moreActions = actionsList.filter(
            (action) => action.for === 'all',
        )[0]

        const renderOptions = () => {
            if (!useActions || showHiddenOptionButtons?.length === 0)
                return null
            ...
            return showHiddenOptionButtons?.map((actionOption, index) => {
                ...
                if (icon) {
                    return (
                        <ToolTip
                            key={`${name}-${index}`}
                            message={`${name} ${name === 'Go to component' ? '' : currentNode}`}
                        >
                            <span
        ...
        return (
            ...
            >
                {enableCopyToClipboard && (
                    <ToolTip message={'Copy to clipboard'}>
                        <span
                            onClick={() => {
                                copyToClipboard(
                                    getAbsoluteNodePath(currentNodePath),
                                )
                            }}
                            style={{
                                height: '13px',
                ...
                <ToolTip message={'Copy value'}>
                    <span
                        onClick={() => {
                            moreActions['actions'][0].dispatchAction(
                                data,
                                currentNode,
                            )
                        }}
    ...
    return (
        <Container
            fluid
            px="1"
            className={cx({ 'json-node-element': !expandable })}
        >
            <Stack direction="row" gap='0'>
                ...
                >
                    <Flex
                        className={cx('inspector-json-node', {
                            'group-object-container':
                                shouldDisplayIntendedBlock,
                            'mx-2':
                                typeofCurrentNode !== 'Object' &&
                                typeofCurrentNode !== 'Array',
                        })}
                    ...
                    >
                        {$NODEIcon && (
                            <Box className="json-tree-icon-container">
                                {$NODEIcon}
                            </Box>
                        )}
                        {$key} {$NODEType}
                        ...
                        <div className="action-icons-group">
                            {useActions && renderHiddenOptionsForNode()}
                        </div>
                    </Flex>
                    {toRenderSelector &&
                        (toExpandNode && !expandable ? null : $VALUE)}
                </Box>
            </Stack>
        </Container>
    )
...
const DisplayNodeLabel = ({ type = '', children }) => {
    ...
    return (
        <>
            <span className="mx-1 text-sm node-type">{type}</span>
            {children}
        </>
    )
}
