...
export default function ObjectNode({
    ...
    const currentPath =
        typeof indexOrName !== 'undefined'
            ? [...parentPath, String(indexOrName)]
            : parentPath
    ...
    const Icons = (
        <>
            ...
            {!fold &&
                !isEditing &&
                editableAdd(editable) &&
                customAdd(customOptions) && (
                    <AddSVG
                        className="json-view--edit"
                        onClick={() => {
                            if (isPlainObject) {
            ...
            {typeof CustomOperation === 'function' ? (
                <CustomOperation node={node} />
            ) : null}
        </>
    )
