...
export const getAutocompletion = (
    ...
    if (fieldType && fieldType !== 'union') {
        ...
        JSLangHints = hints['jsHints'][fieldType]['methods'].map(
            (hint: any) => ({
                hint: hint,
                type: 'js_method',
            }),
        ...
        JSLangHints = Object.keys(hints['jsHints'])
            .map((key) => {
                return hints['jsHints'][key]['methods'].map((hint: any) => ({
                    hint: hint,
                    type: 'js_method',
                }))
            })
    ...
    const appHints = hints['appHints'].filter((cm: any) => {
        const { hint } = cm
    ...
    const autoSuggestionList = appHintsFilteredByDepth.filter(
        (suggestion: any) => {
    ...
    const jsHints = JSLangHints.filter((cm: any) => {
        const lastCharsAfterDot = actualInput.split('.').pop()
        if (cm.hint.includes(lastCharsAfterDot)) return true

        if (autoSuggestionList.length === 0 && !cm.hint.includes(actualInput))
            return true
    })
...
function orderSuggestions(suggestions: any, validationType: any) {
    if (!validationType) return suggestions

    const matchingSuggestions = suggestions.filter(
        (s: any) => s.detail === validationType,
    )
...
export const generateHints = (
    ...
    const suggestions = hints.map(({ hint, type }: any) => {
        let displayedHint =
            type === 'js_method' ||
            (type === 'Function' && !hint.endsWith('.run()'))
                ? `${hint}()`
                : hint
        ...
        return {
            ...
            apply: (view: any, completion: any, from: any, to: any) => {
                ...
                const pickedFrom =
                    actualStartIndex === 0 && end - to > 2
                        ? from - currentWord.length
                        : actualStartIndex + (end - to)
                ...
                const multiReferenceInSingleIndentifier =
                    totalReferences == 1 && searchText !== currentWord
                ...
                const actualInput = removeNestedDoubleCurlyBraces(
                    doc.toString(),
                )
...
function filterHintsByDepth(input: any, hints: any) {
    if (input === '') return hints

    const inputDepth = input.includes('.') ? input.split('.').length : 0

    const filteredHints = hints.filter((cm: any) => {
        const hintParts = cm.hint.split('.')

        let shouldInclude =
            (cm.hint.startsWith(input) &&
                hintParts.length === inputDepth + 1) ||
            (cm.hint.startsWith(input) && hintParts.length === inputDepth)

        const shouldFuzzyMatch = !shouldInclude
            ? hintParts.length > inputDepth
            : false
...
export function findNearestSubstring(inputStr: any, currentCurosorPos: any) {
