...
import {
    ...
    generateHints,
} from './autocompleteExtensionConfig'
import { ErrorBoundary } from '@/views/actors/playground/_components/ErrorBoundary'
import CodeHinter from './CodeHinter'
import { CodeHinterContext } from './CodeHinterContext'
import { createReferencesLookup } from '@/views/actors/playground/_stores/utils'
...
const MultiLineCodeEditor = (props) => {
    ...
    // const replaceIdsWithName = useStore(
    //     (state) => state.replaceIdsWithName,
    //     shallow,
    // )

    const { replaceIdsWithName, getSuggestions } = useEditor()

    // const getSuggestions = useStore((state) => state.getSuggestions, shallow)

    const isInsideQueryPane = !!document
        .querySelector('.code-hinter-wrapper')
        ?.closest('.query-details')
    ...
    function autoCompleteExtensionConfig(context) {
        ...
        const nearestSubstring = removeNestedDoubleCurlyBraces(
            findNearestSubstring(inputStr, currentCurosorPos),
        )
        ...
        const suggestions = generateHints(
            ...
            if (
                hint.label.startsWith('client') ||
                hint.label.startsWith('server')
            )
                return
    ...
    const { handleTogglePopupExapand, isOpen, setIsOpen, forceUpdate } =
        portalProps
    let cyLabel = paramLabel
    ...
    const initialValueWithReplacedIds = useMemo(() => {
        if (
            ...
            (initialValue?.includes('components') ||
                initialValue?.includes('queries'))
        ) {
    ...
    return (
        ...
        >
            <div
                className={`${className} ${darkMode && 'cm-codehinter-dark-themed'}`}
            >
