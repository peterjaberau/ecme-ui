...
import { LayoutOptions } from '@flow/free-auto-layout-plugin'
import { EditorState } from '@flow/editor'
...
type SetCursorStateCallback = (
    e: SetCursorStateCallbackEvent,
) => EditorCursorState | undefined
...
export interface PlaygroundTools {
    ...
    cursorState: EditorCursorState
    setCursorState: (
        stateId: EditorCursorState | SetCursorStateCallback,
    ) => void
...
export function usePlaygroundTools(): PlaygroundTools {
    ...
    useEffect(() => {
        const disposable = playground.editorState.onStateChange((e) => {
            setCursorState(
                e.state === EditorState.STATE_GRAB ||
    ...
    function handleUpdateCursorState(
        stateId: EditorCursorState | SetCursorStateCallback,
    ) {
