...
} from '@flow/free-stack-plugin'
import { createFreeLinesPlugin } from '@flow/free-lines-plugin'
import {
    WorkflowCommands,
    WorkflowNodeEntity,
    WorkflowLineEntity,
...
import { createFreeHoverPlugin } from '@flow/free-hover-plugin'
import {
    HistoryService,
    createFreeHistoryPlugin,
} from '@flow/free-history-plugin'
import { createFreeAutoLayoutPlugin } from '@flow/free-auto-layout-plugin'
import {
    PluginsProvider,
    Plugin,
    createDefaultPreset,
                ...
                registerShortcuts(registry) {
                        ...
                        execute: () => {
                            selection.selection.forEach((entity) => {
                                if (entity instanceof WorkflowNodeEntity) {
                                    ...
                                    const nodeMeta =
                                        entity.getNodeMeta<WorkflowNodeMeta>()
                                    const subCanvas =
                                        nodeMeta.subCanvas?.(entity)
                                    ...
                                    if (
                                        !ctx.document.linesManager.canRemove(
                                            entity,
                                        )
                                    ) {
                                        return
                    ...
                    if (opts?.history?.enable) {
                        const fixedHistoryService =
                            ctx.get<HistoryService>(HistoryService)
        ...
        plugins.push(
            createPlaygroundPlugin<FreeLayoutPluginContext>({
                onBind: (bindConfig) => {
                    bindConfig.rebind(WorkflowDocumentOptions).toConstantValue({
                        ...
                        setLineRenderType: opts.setLineRenderType?.bind(
                            null,
                            ctx,
                        ),
                        setLineClassName: opts.setLineClassName?.bind(
                            null,
                            ctx,
                        ),
                        ...
                        cursors:
                            opts.cursors ??
                            WorkflowDocumentOptionsDefault.cursors,
                        lineColor:
                            opts.lineColor ??
                            WorkflowDocumentOptionsDefault.lineColor,
                ...
                onInit: (ctx) => {
                    ...
                    if (opts.onContentChange) {
                        ctx.document.onContentChange((event) =>
            ...
            createSelectBoxPlugin({
                canSelect: (e) => {
                    ...
                    const hoverService =
                        ctx.get<WorkflowHoverService>(WorkflowHoverService)
