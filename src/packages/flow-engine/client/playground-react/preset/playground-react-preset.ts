import { createShortcutsPlugin } from '@flow/shortcuts-plugin'
import {
    PluginContext,
    PluginsProvider,
    Plugin,
...
import { createBackgroundPlugin } from '@flow/background-plugin'

import { PlaygroundReactProps } from './playground-react-props'

export function createPlaygroundReactPreset<
            ...
            createPlaygroundPlugin<CTX>({
                ...
                onInit: (ctx) => {
                    const playgroundConfig =
                        ctx.get<PlaygroundConfig>(PlaygroundConfig)
                    if (opts.playground) {
                        if (opts.playground.autoFocus !== undefined) {
                            playgroundConfig.autoFocus =
                                opts.playground.autoFocus
                        }
                        if (opts.playground.autoResize !== undefined) {
                            playgroundConfig.autoResize =
                                opts.playground.autoResize
                        }
                    }
