...
import { FlowNodeFormData, FormMetaOrFormMetaGenerator } from '@flow/form-core'
import {
    FlowDocument,
    FlowDocumentJSON,
    FlowNodeEntity,
    ...
    export const DEFAULT: EditorProps = {
        ...
        fromNodeJSON(node: FlowNodeEntity, json: FlowNodeJSON) {
            ...
            if (!formData) {
                ...
                const formMeta =
                    node.getNodeRegistry()?.formMeta || defaultFormMeta
        ...
        toNodeJSON(node: FlowNodeEntity): FlowNodeJSON {
            ...
            node.document.traverse((node) => {
                ...
                let formJSON =
                    formData &&
                    formData.formModel &&
                    formData.formModel.initialized
                        ? formData.toJSON()
                        : undefined
                const nodeJSON = {
                    id: node.id,
