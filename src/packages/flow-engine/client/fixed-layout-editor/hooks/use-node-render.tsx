...
import { useObserve } from '@flow/reactive'
import { useStartDragNode } from '@flow/fixed-drag-plugin'
import {
    usePlayground,
    FlowNodeBaseType,
    FlowNodeEntity,
...
export function useNodeRender(
    ...
    const renderNode =
        nodeFromProps || useContext<FlowNodeEntity>(PlaygroundEntityContext)
    const renderData =
    ...
    const isBlockOrderIcon =
        renderNode.flowNodeType === FlowNodeBaseType.BLOCK_ORDER_ICON
    const isBlockIcon = renderNode.flowNodeType === FlowNodeBaseType.BLOCK_ICON
    const node =
        isBlockOrderIcon || isBlockIcon ? renderNode.parent! : renderNode
    const operationService =
        useService<FlowOperationService>(FlowOperationService)
    ...
    useEffect(() => {
        ...
        if (isBlockIcon || isBlockOrderIcon) {
            // icon 的扩展数据是存在父节点上
            dispose = renderNode.parent!.onExtInfoChange(() =>
                renderNode.renderData.fireChange(),
            )
        }
