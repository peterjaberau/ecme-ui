...
import { FlowOperationBaseService } from '@flow/canvas-document'
...
import { HistoryContainerModule, OperationService } from '@flow/history'
import { OperationContribution } from '@flow/history'
...
export function registerHistory(
    ...
    rebind(FlowOperationBaseService)
        .to(FixedHistoryOperationService)
        .inSingletonScope()
}

export const createFixedHistoryPlugin = definePluginCreator<
    FixedHistoryPluginOptions<any>
>({
    onBind: ({ bind, rebind }) => {
    ...
    onInit(ctx, opts): void {
        ...
        if (opts?.getDocumentJSON) {
            fixedHistoryService.historyService.config.getSnapshot =
                opts.getDocumentJSON(ctx)
        } else {
            fixedHistoryService.historyService.config.getSnapshot = () =>
                document.toJSON()
        }
        ...
        document.onNodeCreate(({ node, data }) => {
            ...
            formData.formModel.onInitialized(() => {
                formData.onDetailChange((event) => {
                    ...
                    const value = fixedFormDataService.getFormItemValue(
                        formData,
                        path,
                    )
                    const oldValue = fixedFormDataService.getCache(
                        formData,
                        path,
                    )
