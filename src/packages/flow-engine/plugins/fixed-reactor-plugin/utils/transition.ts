...
import { IPoint, Point } from '@flow/utils'
 ...
 * @returns
 */
export const getReactorChildLineStartPoint = (
    iconTransform?: FlowNodeTransformData,
): IPoint => {
    if (!iconTransform) {
        return { x: 0, y: 0 }
    }
 ...
 * @returns
 */
export const getTransitionToPoint = (
...
export const drawCollapseLabel = (
    transition: FlowNodeTransitionData,
): FlowTransitionLabel[] => {
 ...
 * @returns
 */
export const drawStraightAdder = (
    transition: FlowNodeTransitionData,
): FlowTransitionLabel[] => {
    const toPoint = getTransitionToPoint(transition)
