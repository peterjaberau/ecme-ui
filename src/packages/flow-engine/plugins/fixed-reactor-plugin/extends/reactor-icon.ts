import { FlowNodeRegistry, FlowNodeBaseType } from '@flow/canvas-document'
...
export const reactorIcon: FlowNodeRegistry = {
    ...
    getLines: (transition) => [
        ...(canReactorDrilldown(transition.entity.parent!)
            ? drawCollapseLine(transition)
            : []),
    ],
    getLabels: (transition) => [
        ...(canReactorDrilldown(transition.entity.parent!)
            ? drawCollapseLabel(transition)
            : []),
    ],
    getDelta: (transform) => {
