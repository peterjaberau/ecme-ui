import { FlowNodeRegistry } from '@flow/canvas-document'
...
export const Reactor: FlowNodeRegistry = {
    ...
    meta: {
        ...
        padding: (node) => ({
            ...
            bottom:
                !insideReactor(node.entity) && node.isLast
                    ? REACTOR_NODE_LAST_SPACING
                    : 0,
            top: 0,
        }),
    ...
    getLines: (transition) => [
        ...(!insideReactor(transition.entity)
            ? drawStraightLine(transition)
            : []),
    ],
    getLabels: (transition) => [
        ...(!insideReactor(transition.entity)
            ? drawStraightAdder(transition)
            : []),
    ],
    getInputPoint,
    getOutputPoint,
    onAfterUpdateLocalTransform(transform) {
        ...
        if (
            iconSize / 2 + portsMiddle > inlineBlocksSize ||
            !inlineBlocks.children.length
        ) {
