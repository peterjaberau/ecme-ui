...
import { FlowNodeMeta } from '@flow/canvas-document'
...
import {
    RENDER_REACTOR_COLLAPSE_KEY,
    RENDER_REACTOR_PORT_KEY,
} from './constants'

export type MaterialReactComponent<T = any> = (
    props: T,
...
export const createFixedReactorPlugin =
    definePluginCreator<FixedReactorPluginOpts>({
        onBind({ bind }, opts) {
            // 如果要收缩，则注册收缩布局优化逻辑
            if (opts?.shrink) {
                bindContributions(bind, ReactorShrinkLayout, [
                    FlowLayoutContribution,
                ])
            }
        },
        onInit(ctx, opts): void {
            ...
            const registry: FlowRendererRegistry =
                ctx.get<FlowRendererRegistry>(FlowRendererRegistry)
            ...
            document.registerFlowNodes({
                ...
                ...(transformFlowNodeJSON
                    ? {
                          onCreate: (node, json) =>
                              createReactorFromJSON(
                                  node,
                                  transformFlowNodeJSON(json),
                              ),
                      }
                    : {}),
            ...
            registry.registerReactComponent(
                RENDER_REACTOR_PORT_KEY,
                opts.renderReactorPort,
            )
            registry.registerReactComponent(
                RENDER_REACTOR_COLLAPSE_KEY,
                opts.renderReactorCollapse,
            )
        },
    })
