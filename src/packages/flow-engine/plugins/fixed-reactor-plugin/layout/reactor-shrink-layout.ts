...
import { FlowLayoutDefault } from '@flow/canvas-document'
...
export class ReactorShrinkLayout implements FlowLayoutContribution {
    ...
    layoutReactorShrinkVertical(transform: FlowNodeTransformData) {
        ...
        if (isReactor(pre?.entity)) {
            ...
            const iconRight = icon?.localBounds.right || 0
            const y =
                (pre.localBounds?.top || 0) +
                (icon?.localBounds.bottom || 0) +
                (pre.spacing || 0)
        ...
        if (
            preShrinkData &&
            transform.localBounds.top <= preShrinkData.blockEnd
        ) {
    ...
    layoutReactorShrinkInHorizontal(transform: FlowNodeTransformData) {
        ...
        if (isReactor(pre?.entity)) {
            ...
            const x =
                (pre.localBounds?.left || 0) +
                (icon?.localBounds.right || 0) +
                (pre.spacing || 0)
        ...
        if (
            preShrinkData &&
            transform.localBounds.left <= preShrinkData.blockEnd
        ) {
