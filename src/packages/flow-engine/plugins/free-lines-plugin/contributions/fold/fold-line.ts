import { type IPoint, Point, Rectangle } from '@flow/utils'
...
export namespace FoldLine {
    ...
    function getEdgeCenter({
        ...
        const centerX =
            target.x < source.x ? target.x + xOffset : target.x - xOffset

        const yOffset = Math.abs(target.y - source.y) / 2
        const centerY =
            target.y < source.y ? target.y + yOffset : target.y - yOffset
    ...
    const getDirection = ({
        ...
        source: IPoint
        target: IPoint
    }): IPoint => (source.x < target.x ? { x: 1, y: 0 } : { x: -1, y: 0 })
    ...
    export function getPoints({
        ...
        if (sourceDir[dirAccessor] * targetDir[dirAccessor] === -1) {
            ...
            const sourceTarget: IPoint[] = [
                { x: sourceGapped.x, y: targetGapped.y },
            ]
            const targetSource: IPoint[] = [
     ...
     * @param pos
     */
    export function getFoldLineToPointDistance(
        points: IPoint[],
        pos: IPoint,
    ): number {
