...
import { FlowNodeEntity, FlowNodeTransformData } from '@flow/canvas-document'
import { FlowNodeBaseType } from '@flow/canvas-document'
import { FlowDocument } from '@flow/canvas-document'
import { EntityManager, PlaygroundConfigEntity } from '@flow/canvas-core'
...
export class FlowMinimapService {
    ...
    private createRenderContext(): MinimapRenderContext {
        ...
        const nodeTransforms: FlowNodeTransformData[] =
            this.nodeTransforms(nodes)
        const nodeRects: Rectangle[] = nodeTransforms.map(
    ...
    private renderCanvas(renderContext: MinimapRenderContext) {
        const { canvas, context2D, nodeRects, viewRect, scale, offset } =
            renderContext
    ...
    private nodeTransforms(nodes: FlowNodeEntity[]): FlowNodeTransformData[] {
        return nodes
            .map((node) => node.getData(FlowNodeTransformData))
            .filter(Boolean)
    }
    ...
    private mountListener(): void {
        const entityManagerDisposer = this.entityManager.onEntityChange(() =>
            this.render(),
        )
    ...
    private handleStartDrag = (event: MouseEvent): void => {
        ...
        if (
            !this.isPointInRect({ point: mousePoint, rect: viewRectOnCanvas })
        ) {
            return
        }
