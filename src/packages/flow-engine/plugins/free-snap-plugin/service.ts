...
import { Disposable, Emitter, Rectangle } from '@flow/utils'
import { IPoint } from '@flow/utils'
import { WorkflowNodeEntity, WorkflowDocument } from '@flow/free-layout-core'
import { WorkflowDragService } from '@flow/free-layout-core'
import { FlowNodeTransformData } from '@flow/canvas-document'
import { FlowNodeBaseType } from '@flow/canvas-document'
import {
    EntityManager,
...
export class WorkflowSnapService {
    ...
    private mountListener(): void {
        const dragAdjusterDisposer = this.dragService.registerPosAdjuster(
            (params: any) => {
                ...
                if (
                    this._disabled ||
                    !this.options.enableEdgeSnapping ||
                    isMultiSnapping
                ) {
                    return {
                        x: 0,
    ...
    private calcSnapOffset(params: {
        ...
        const snapEdgeLines: SnapEdgeLines = {
            top: isEqual(topYClosest, snappingPosition.y)
                ? topYClosestLine
                : undefined,
            bottom: isEqual(bottomYClosest, snappingPosition.y)
                ? bottomYClosestLine
                : undefined,
            left: isEqual(leftXClosest, snappingPosition.x)
                ? leftXClosestLine
                : undefined,
            right: isEqual(rightXClosest, snappingPosition.x)
                ? rightXClosestLine
                : undefined,
            midVertical: isEqual(midXClosest, snappingPosition.x)
                ? midXClosestLine
                : undefined,
            midHorizontal: isEqual(midYClosest, snappingPosition.y)
                ? midYClosestLine
                : undefined,
        }
    ...
    private getAvailableNodes(params: {
        ...
        const targetContainerId =
            targetNodes[0].parent?.id ?? this.document.root.id
        ...
        const availableNodes = this.nodes
            ...
            .sort((nodeA, nodeB) => {
                const nodeCenterA = nodeA.getData(FlowNodeTransformData)!.bounds
                    .center
                const nodeCenterB = nodeB.getData(FlowNodeTransformData)!.bounds
                    .center
                // 距离越近优先级越高
                const distanceA =
                    Math.abs(nodeCenterA.x - targetCenter.x) +
                    Math.abs(nodeCenterA.y - targetCenter.y)
                const distanceB =
                    Math.abs(nodeCenterB.x - targetCenter.x) +
                    Math.abs(nodeCenterB.y - targetCenter.y)
    ...
    private getBounds(nodes: WorkflowNodeEntity[]): Rectangle {
        ...
        return Rectangle.enlarge(
            nodes.map((n) => n.getData(FlowNodeTransformData)!.bounds),
        )
    }

    private updateNodePositionWithOffset(params: {
    ...
    private calcAlignOffset(params: {
        ...
        if (alignSpacing.top) {
            ...
            const isAlignTop = isLessThanOrEqual(
        ...
        if (alignSpacing.bottom) {
            const bottomAlignY =
                alignRects.bottom[0].rect.top - alignSpacing.bottom
            const isAlignBottom = isLessThan(
                Math.abs(targetRect.bottom - bottomAlignY),
                alignThreshold,
            )
            if (isAlignBottom) {
        ...
        if (alignSpacing.left) {
            ...
            const isAlignLeft = isLessThanOrEqual(
        ...
        if (alignSpacing.right) {
            const rightAlignX =
                alignRects.right[0].rect.left - alignSpacing.right
        ...
        if (alignSpacing.midHorizontal) {
            const leftAlignX =
                alignRects.left[0].rect.right + alignSpacing.midHorizontal
        ...
        if (alignSpacing.midVertical) {
            const topAlignY =
                alignRects.top[0].rect.bottom + alignSpacing.midVertical
    ...
    private intersection(
        ...
        const isVerticalIntersection =
            isLessThan(rectA.left, rectB.right) &&
            isGreaterThan(rectA.right, rectB.left)
        const isHorizontalIntersection =
            isLessThan(rectA.top, rectB.bottom) &&
            isGreaterThan(rectA.bottom, rectB.top)
        const isIntersection =
            isHorizontalIntersection && isVerticalIntersection
