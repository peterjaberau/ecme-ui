...
export class WorkflowSnapLayer extends Layer<WorkflowSnapLayerOptions> {
        ...
        return (
            <>
                {this.alignLines.length > 0 && (
                    <div className="workflow-snap-align-lines">
                        {this.renderAlignLines()}
                    </div>
                )}
                {this.edgeLines.length > 0 && (
                    <div className="workflow-snap-edge-lines">
                        {this.renderEdgeLines()}
                    </div>
                )}
            </>
        )
    ...
    private renderAlignLines(): JSX.Element[] {
        return this.alignLines.map((renderLine: SnapRenderLine) => {
            ...
            // 调整渲染位置以保持居中
            const adjustedTop = isHorizontal
                ? renderLine.top - alignLineWidth / 2
                : renderLine.top
            const adjustedLeft = isHorizontal
                ? renderLine.left
                : renderLine.left - alignLineWidth / 2
                ...
                >
                    ...
                    <div
                        style={{
                            ...
                            top: isHorizontal
                                ? adjustedTop -
                                  (alignCrossWidth - alignLineWidth) / 2
                                : adjustedTop,
                            left: isHorizontal
                                ? adjustedLeft
                                : adjustedLeft -
    ...
    private calcEdgeLines(event: SnapEvent): SnapRenderLine[] {
        ...
        // 处理底部对齐
        if (bottomFullAlign) {
            const top = topFullAlign
                ? snapRect.top + snapRect.height / 2
                : snapRect.top
        ...
        const snappedEdgeLines = Object.entries(snapEdgeLines)
            .map(([direction, snapLine]) => {
                ...
                const nodeRect = sourceNode.getData(
    ...
    private directionFullAlign(params: {
        ...
        for (let i = 0; i < alignRects.length; i++) {
            ...
            const isFullAlign = this.rectFullAlign(
                alignRect.rect,
                prevRect,
                isVertical,
            )
    ...
    private rectFullAlign(
        ...
        if (isVertical) {
            return (
                isEqual(rectA.left, rectB.left) &&
                isEqual(rectA.right, rectB.right)
            )
        } else {
            return (
                isEqual(rectA.top, rectB.top) &&
                isEqual(rectA.bottom, rectB.bottom)
            )
        }
    }
    ...
    private calcDirectionAlignLines(params: {
        ...
        for (let i = 0; i < alignRects.length; i++) {
            ...
            const betweenSpacing = isVertical
                ? Math.min(
                      Math.abs(prevRect.top - rect.bottom),
                      Math.abs(prevRect.bottom - rect.top),
                  )
                : Math.min(
                      Math.abs(prevRect.left - rect.right),
                      Math.abs(prevRect.right - rect.left),
                  )
            ...
            if (isVertical) {
                const centerX = this.calcHorizontalIntersectionCenter(
                    rect,
                    targetRect,
                )
                alignLines.push({
                ...
                const centerY = this.calcVerticalIntersectionCenter(
                    rect,
                    targetRect,
                )
                alignLines.push({
    ...
    private calcVerticalIntersectionCenter(
        rectA: Rectangle,
        rectB: Rectangle,
    ): number {
    ...
    private calcHorizontalIntersectionCenter(
        rectA: Rectangle,
        rectB: Rectangle,
    ): number {
