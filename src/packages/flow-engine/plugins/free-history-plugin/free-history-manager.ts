...
import { DisposableCollection, Disposable } from '@flow/utils'
...
import { OperationType } from '@flow/canvas-document'
import { type PluginContext, PositionData } from '@flow/canvas-core'

import {
    DragNodeOperationValue,
...
export class FreeHistoryManager {
    ...
    onInit(ctx: PluginContext, opts: FreeHistoryPluginOptions) {
        ...
        const resetLayoutService = ctx.get<WorkflowResetLayoutService>(
            WorkflowResetLayoutService,
        )
        ...
        this._toDispose.pushAll([
            ...
            this._formManager
                ? this._formManager.onFormModelWillInit(({ model, data }) => {
                      ...
                      const formData =
                          node.getData<FlowNodeFormData>(FlowNodeFormData)

                      if (formData) {
                          this._entityManager.setValue(
                              formData,
                              cloneDeep(data),
                          )
