...
const assignCutValue = (
    ...
    const edge = t.edges.find(
        (e) => e.from === childId && e.to === child.parent,
    )
    if (edge) {
...
const calcCutValue = (
    ...
    const graphEdge = g.edges.find(
        (e) =>
            (e.from === childId && e.to === child.parent) ||
...
const isDescendant = (
    t: LayoutGraph,
    vLabel: LayoutNode,
    rootLabel: LayoutNode,
): boolean =>
    (rootLabel.low || 0) <= (vLabel.lim || 0) &&
    (vLabel.lim || 0) <= (rootLabel.lim || 0)
...
const updateRanks = (t: LayoutGraph, g: LayoutGraph): void => {
    ...
    vs.slice(1).forEach((v) => {
        ...
        const edge = g.edges.find(
            (e) =>
                (e.from === v && e.to === node.parent) ||
                (e.from === node.parent && e.to === v),
        )
        if (!edge) return

        const flipped = edge.from === node.parent
        node.rank =
            parent.rank + (flipped ? edge.minlen || 1 : -(edge.minlen || 1))
    })
}
