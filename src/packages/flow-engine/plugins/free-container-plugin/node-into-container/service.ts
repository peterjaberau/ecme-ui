...
export class NodeIntoContainerService {
    ...
    /** 将节点移出容器 */
    public async moveOutContainer(params: {
    ...
    private async dragOutContainer(event: NodesDragEvent): Promise<void> {
        ...
        const activated =
            event.triggerEvent.metaKey || event.triggerEvent.ctrlKey
        if (
            !activated || // 必须按住指定按键
            !dragNode || // 必须有一个节点
    ...
    private getCollisionTransform(params: {
        ...
        const collisionTransform = transforms.find((transform) => {
            ...
            const padding = withPadding
                ? this.document.layout.getPadding(entity)
                : { left: 0, right: 0 }
    ...
    private draggingNode(nodeDragEvent: NodesDragEvent): void {
        ...
        if (
            !isDraggingNode ||
            !dragNode ||
            this.isContainer(dragNode) ||
            !transforms?.length
        ) {
            return this.setDropNode(undefined)
        }
        const mousePos = this.playgroundConfig.getPosFromMouseEvent(
            nodeDragEvent.dragEvent,
        )
