import { IPoint, PaddingSchema, Point } from '@flow/utils'
import {
    FlowGroupController,
    FlowNodeBaseType,
    FlowNodeRegistry,
...
export const GroupRegister: FlowNodeRegistry = {
    ...
    meta: {
        ...
        padding: (transform: FlowNodeTransformData): PaddingSchema => {
            ...
            if (
                !groupController ||
                groupController.collapsed ||
                groupController.nodes.length === 0
            ) {
                return {
                    top: 0,
                    bottom: 0,
            ...
            return [
                {
                    offset: Point.getMiddlePoint(
                        Point.move(transform.outputPoint, {
                            x: PositionConfig.paddingWithAddLabel / 2,
                            y: 0,
                        }),
                        transform.next.inputPoint,
                    ),
    ...
    getOriginDeltaY(transform): number {
        ...
        // 这里要加上 y 轴的偏移
        return (
            -transform.size.height * transform.origin.y -
            PositionConfig.paddingWithNote
        )
    },
}
