...
import { SelectorBoxConfigEntity } from '@flow/canvas-renderer'
import {
    WorkflowDocument,
    WorkflowDragService,
...
export class HoverLayer extends Layer<HoverLayerOptions> {
    ...
    onReady(): void {
        ...
        this.toDispose.pushAll([
            ...
            this.listenPlaygroundEvent('mousemove', (e: MouseEvent) => {
                this.hoverService.hoveredPos =
                    this.config.getPosFromMouseEvent(e)
                if (!this.isEnabled()) {
                    return
                }
            ...
            this.listenPlaygroundEvent(
                ...
                (e: MouseEvent): boolean | undefined => {
                    ...
                    if (
                        hoveredNode &&
                        hoveredNode instanceof WorkflowLineEntity
                    ) {
                        ...
                        this.dragService
                            ...
                            ?.then((dragSuccess) => {
                                if (!dragSuccess) {
                                    ...
                                    if (
                                        hoveredNode &&
                                        hoveredNode instanceof
                                            WorkflowNodeEntity
                                    ) {
                                        // 追加选择
                                        if (e.shiftKey) {
                                            this.selectionService.toggleSelect(
                                                hoveredNode,
                                            )
                                        } else {
                                            this.selectionService.selectNode(
                                                hoveredNode,
                                            )
                                        }
                                    } else {
    ...
    updateHoveredState(mousePos: IPoint, target?: HTMLElement): void {
        ...
        const lineDomNodes =
            this.playgroundNode.querySelectorAll(LINE_CLASS_NAME)
        ...
        const nodeHovered = nodeTransforms.find(
            (trans: FlowNodeTransformData) =>
                trans.bounds.contains(mousePos.x, mousePos.y),
        )?.entity as WorkflowNodeEntity

        // 判断当前鼠标位置所在元素是否在节点内部
        const nodeDomNodes =
            this.playgroundNode.querySelectorAll(NODE_CLASS_NAME)
