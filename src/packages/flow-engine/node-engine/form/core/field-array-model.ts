import { Emitter } from '@flow/utils'

import { FieldValue } from '../types'
import { Path } from './path'
import { FieldModel } from './field-model'

export class FieldArrayModel<TValue = FieldValue> extends FieldModel<
    Array<TValue>
> {
    ...
    _splice(start: number, deleteCount = 1) {
        ...
        const recursiveHandleChildField = (
            field: FieldModel,
            index: number,
        ) => {
            ...
            // 剩余的项 index 向前移动 {deleteCount} 位， 并触发变更事件
                ...
                const targetName = field.path
                    .replaceParent(
                        ...
                        this.path.concat(index - deleteCount),
                    )
                    .toString()
    ...
    swap(from: number, to: number) {
        ...
        if (
            from < 0 ||
            to < 0 ||
            from > this.value.length - 1 ||
            to > this.value.length - 1
        ) {
            throw new Error(
        ...
        fromFields.forEach((f) => {
            const newName = f.path
                .replaceParent(fromRootPath, toRootPath)
                .toString()
            f.name = newName
        ...
        toFields.forEach((f) => {
            const newName = f.path
                .replaceParent(toRootPath, fromRootPath)
                .toString()
            f.name = newName
            if (!f.children.length) {
                f.updateNameForLeafState(newName)
    ...
    protected insertAt(index: number, value: TValue) {
        ...
        if (index < 0 || index > this.value.length) {
            throw new Error(
                `[Form]: FieldArrayModel.insertAt Error: index exceeds array boundary`,
            )
        }
