...
import { ReactiveState } from '@flow/reactive'

import {
    feedbackToFieldErrorsOrWarnings,
    hasError,
    toFeedback,
} from '../utils/validate'
...
export class FormModel<TValues = any> implements Disposable {
    ...
    readonly onFormValuesChangeEmitter =
    ...
    readonly onFormValuesUpdatedEmitter =
        new Emitter<OnFormValuesUpdatedPayload>()
    ...
    protected _state: ReactiveState<FormModelState> =
        new ReactiveState<FormModelState>(createFormModelState())
    ...
    createFieldArray<TValue = FieldValue>(
        ...
        return this.createField<Array<TValue>>(
            name,
            true,
        ) as FieldArrayModel<TValue>
    }
    ...
    getField<TFieldModel extends FieldModel | FieldArrayModel = FieldModel>(
        ...
        return this.fieldMap.get(new Path(name).toString()) as
            | TFieldModel
            | undefined
    }
    ...
    async validate(): Promise<FormValidateReturn> {
        ...
        const feedbacksArrPromises = Object.keys(this._options.validate).map(
            async (nameRule) => {
                const validate = this._options.validate![nameRule]
                const paths = Glob.findMatchPathsWithEmptyValue(
                    this.values,
                    nameRule,
                )
                return Promise.all(
                    paths.map(async (path) => {
                        ...
                        const warnings =
                            feedbackToFieldErrorsOrWarnings<Warnings>(
                                path,
                                feedback,
                            )
                        ...
                        this.state.errors = mergeFeedbacks(
                            this.state.errors,
                            errors,
                        )
                        this.state.warnings = mergeFeedbacks(
                            this.state.warnings,
                            warnings,
                        )

                        this.state.invalid = !getValidByErrors(
                            this.state.errors,
                        )
                        return feedback
                    }),
