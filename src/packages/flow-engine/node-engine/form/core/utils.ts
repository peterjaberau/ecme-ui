...
export function mergeFeedbacks<T extends Errors | Warnings>(
    origin?: T,
    source?: T,
) {
    if (!source) {
...
export function clearFeedbacks<T extends Errors | Warnings>(
    name: string,
    origin?: T,
) {
    if (!origin) {
...
export function shouldValidate(
    currentTrigger: ValidateTrigger,
    formTrigger?: ValidateTrigger,
) {
...
export function getValidByErrors(errors: Errors | undefined) {
    return errors
        ? Object.keys(errors).every((name) => isEmpty(errors[name]))
        : true
}
