...
export namespace Glob {
    ...
    export function getSubPaths(paths: string[], obj: any): string[] {
        ...
        return flatten(
            paths.map((path) => {
                ...
                if (isArray(value)) {
                    return value.map((_: any, index: number) =>
                        concatPath(path, index),
                    )
    ...
    export function findMatchPaths(
        obj: any,
        pattern: string,
            ...
            if (curKey === ALL) {
                ...
                return flatten(
                    Object.keys(curValue).map((key) => {
                        ...
                        return findMatchPaths(
                            curValue[key],
                            `${nextPaths.join(DIVIDER)}`,
                            withEmptyValue,
                        ).map((p) => concatPath(parentPath, key, p))
                    }),
                )
            }
            // 找不到对应 key 则不匹配
     ...
     * @param pattern
     */
    export function findMatchPathsWithEmptyValue(
        obj: any,
        pattern: string,
    ): string[] {
        if (!pattern.includes('*')) {
