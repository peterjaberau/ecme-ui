...
import { ConfigEntity, type EntityOpts } from '@flow/canvas-core'
...
> {
        ...
        this.document.renderTree.traverse((node, depth, index) => {
            ...
            if (node.parent) {
                transform.setParentTransform(
                    node.parent!.getData(FlowNodeTransformData),
                )
            }
            // 更新 index
            node.index = index
    ...
    isTreeDirty(): boolean {
        const transformVersion = this.entityManager.getEntityDataVersion(
            FlowNodeTransformData,
        )
        const isTreeVersionChanged =
            this.lastTreeVersion !== this.config.treeVersion
        const isTransformVersionChanged =
            this.lastTransformVersion !== transformVersion
    ...
    refresh(): void {
        ...
        if (isTreeVersionChanged || isTransformVersionChanged) {
            ...
            this.lastTransformVersion = this.entityManager.getEntityDataVersion(
                FlowNodeTransformData,
            )
            this.lastTreeVersion = this.config.treeVersion
