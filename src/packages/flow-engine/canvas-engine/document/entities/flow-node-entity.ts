import { Event, type Rectangle } from '@flow/utils'
...
export class FlowNodeEntity extends Entity<FlowNodeEntityConfig> {
    ...
    getNodeRegistry<M extends FlowNodeRegistry = FlowNodeRegistry>(): M {
        ...
        this._registerCache = this.document.getNodeRegistry(
            this.flowNodeType,
            this.originParent,
        )
    ...
    getNodeMeta<M extends FlowNodeMeta = FlowNodeMeta>(): M &
        Required<FlowNodeMeta> {
        if (this._metaCache)
        ...
        if (this.metaFromJSON) {
            ...
            this._metaCache = this.getNodeRegistry().meta as M &
                Required<FlowNodeMeta>
        }
        return this._metaCache as M & Required<FlowNodeMeta>
    }
