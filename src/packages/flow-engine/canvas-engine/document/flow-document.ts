...
import { type Disposable, Emitter } from '@flow/utils'
import {
    type EntityData,
...
export class FlowDocument<T = FlowDocumentJSON> implements Disposable {
    ...
    init(): void {
        ...
        this.renderTree = new FlowRenderTree<FlowNodeEntity>(
            this.root,
            this.originTree,
            this,
        )
    ...
    get layout(): FlowLayout {
        const layout = this.layouts.find(
            (layout) => layout.name == this.currentLayoutKey,
        )
        if (!layout) {
    ...
    fireRender(): void {
        if (this.transformer.isTreeDirty()) {
            ...
            this.entityManager.fireEntityChanged(
                FlowDocumentTransformerEntity.type,
            )
        }
    }
     ...
     */
    addFromNode(
        fromNode: FlowNodeEntity | string,
        json: FlowNodeJSON,
    ): FlowNodeEntity {
        const node =
            typeof fromNode === 'string' ? this.getNode(fromNode)! : fromNode
        this.entityManager.changeEntityLocked = true
        const { parent } = node
    ...
    addNode(
        ...
        if (!node) {
            ...
            node.onDispose(() =>
                this.onNodeDisposeEmitter.fire({ node: node! }),
            )
        ...
        if (register.onCreate) {
            ...
            if (!data.blocks[0].type) {
                ...
                this.addBlocksAsChildren(
                    node,
                    data.blocks as FlowNodeJSON[],
                    addedNodes,
                )
            }
        }
    ...
    addBlock(
        ...
        const node: FlowNodeEntity =
            typeof target === 'string' ? this.getNode(target)! : target
        ...
        if (blockData.blocks) {
            this.addBlocksAsChildren(
                block,
                blockData.blocks as FlowNodeJSON[],
                addedNodes,
            )
        }
        return block
     ...
     * @param registries
     */
    registerFlowNodes<T extends FlowNodeRegistry<any>>(
        ...registries: T[]
    ): void {
        registries.forEach((newRegistry) => {
            if (!newRegistry) {
                throw new Error(
                    '[FlowDocument] registerFlowNodes parameters get undefined registry.',
                )
            }
    ...
    getNodeRegistry<T extends FlowNodeRegistry = FlowNodeRegistry>(
        ...
        const customDefaultRegistry =
            this.options.getNodeDefaultRegistry?.(type)
        ...
        if (originParent) {
            ...
            if (extendRegister) {
                if (
                    extendRegister.extend &&
                    this.registers.has(extendRegister.extend)
                ) {
                    extendRegisters.push(
                        this.registers.get(extendRegister.extend)!,
                    )
                }
        ...
        defaultNodeMeta.spacing =
            this.options?.constants?.[ConstantKeys.NODE_SPACING] ||
            defaultNodeMeta.spacing
