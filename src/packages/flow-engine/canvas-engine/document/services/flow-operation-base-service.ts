...
export class FlowOperationBaseServiceImpl implements FlowOperationBaseService {
    ...
    moveNode(node: FlowNodeEntityOrId, config: MoveNodeConfig = {}) {
        ...
        let toIndex =
            typeof index === 'undefined'
                ? newParentEntity.collapsedChildren.length
                : index
    ...
    dragNodes({
        ...
        const fromIndex = fromParent.children.findIndex(
            (child) => child === startNode,
        )
        const dropIndex = toParent.children.findIndex(
            (child) => child === dropNode,
        )
    ...
    apply(operation: FlowOperation): any {
        ...
        switch (operation.type) {
            case OperationType.addFromNode:
                return document.addFromNode(
                    operation.value.fromId,
            ...
            case OperationType.createGroup: {
                const groupNode = document.addFromNode(
                    operation.value.targetId,
                    {
            ...
            case OperationType.addChildNode: {
                return document.addNode({
                    ...operation.value.data,
                    parent: operation.value.parentId
                        ? document.getNode(operation.value.parentId)
                        : undefined,
    ...
    protected doMoveNode(
        node: FlowNodeEntity,
        newParent: FlowNodeEntity,
        index: number,
    ) {
        if (!node.parent) {
