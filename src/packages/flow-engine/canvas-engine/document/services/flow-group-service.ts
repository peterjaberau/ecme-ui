...
export class FlowGroupService {
    ...
    public createGroup(nodes: FlowNodeEntity[]): FlowNodeEntity | undefined {
        ...
        const sortedNodes: FlowNodeEntity[] = nodes.sort(
            (a, b) => a.index - b.index,
        )
...
export class FlowGroupController {
    ...
    public get collapsed(): boolean {
        const groupTransformData =
            this.groupNode.getData<FlowNodeTransformData>(FlowNodeTransformData)
    ...
    public get bounds(): Rectangle {
        const groupNodeBounds = this.groupNode.getData<FlowNodeTransformData>(
    ...
    private set collapsed(collapsed: boolean) {
        ...
        if (groupTransformData.parent)
            groupTransformData.parent.localDirty = true
    ...
    public set hovered(hovered: boolean) {
        const groupRenderData =
            this.groupNode.getData<FlowNodeRenderData>(FlowNodeRenderData)
        if (hovered) {
    ...
    public static validate(nodes: FlowNodeEntity[]): boolean {
        ...
        // 判断是否有分组节点
        const isGroupRelatedNode = nodes.some((node) =>
            FlowGroupController.isGroupNode(node),
        )
        ...
        const parentsInGroup = parents.some((parent) =>
            this.isNodeInGroup(parent),
        )
    ...
    /** 获取节点分组控制 */
    public static getNodeGroupController(
        node?: FlowNodeEntity,
    ): FlowGroupController | undefined {
        if (!node) {
            return
