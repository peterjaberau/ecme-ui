...
export class FlowDragService {
    ...
    get dragging(): boolean {
        const renderData =
            this.dragStartNode?.getData<FlowNodeRenderData>(FlowNodeRenderData)!
    ...
    isDroppableNode(node: FlowNodeEntity) {
        ...
        if (hasGroupNode) {
            const group =
                FlowGroupController.getNodeRecursionGroupController(node)
            if (group) {
                return false
            }
        }
    ...
    isDroppableBranch(
        node: FlowNodeEntity,
        side: LABEL_SIDE_TYPE = LABEL_SIDE_TYPE.NORMAL_BRANCH,
    ) {
        // 拖拽的是分支
