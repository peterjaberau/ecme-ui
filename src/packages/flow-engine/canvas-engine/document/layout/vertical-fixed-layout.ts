...
function isStructDataEqual(
    ...
    return (
        struct1.childrenLength === struct2.childrenLength &&
...
export class VerticalFixedLayout implements FlowLayout {
    ...
    protected structDataMap = new WeakMap<
        FlowNodeEntity,
        FlowNodeTransformStructData
    >()
    ...
    updateLocalTransform(node: FlowNodeEntity, forceChange = false): boolean {
        ...
        if (children.length > 0) {
            for (const child of children) {
                const childDirty = this.updateLocalTransform(
                    child,
                    siblingDirty,
                )
                // 子节点变更则父节点跟着变更
        ...
        transform.transform.update({
            origin: getOrigin
                ? getOrigin(transform, this)
                : this.getDefaultNodeOrigin(),
        })
        const preTransform = transform.pre
        const delta = getDelta?.(transform, this) || { x: 0, y: 0 }
        const inlineSpacingPre =
            isInlineBlock && transform.parent?.inlineSpacingPre
                ? transform.parent?.inlineSpacingPre
                : 0
        const fromParentDelta = parent
            ?.getNodeRegistry()
            .getChildDelta?.(transform, this) || {
            x: 0,
            y: 0,
        }
    ...
    onAfterUpdateLocalTransform(transform: FlowNodeTransformData) {
        ...
        const { onAfterUpdateLocalTransform } =
            transform.entity.getNodeRegistry()
    ...
    getPadding(node: FlowNodeEntity): PaddingSchema {
        const { inlineSpacingPre, inlineSpacingAfter, padding } =
            node.getNodeMeta()
        ...
        const paddingPre =
            typeof inlineSpacingPre === 'function'
                ? inlineSpacingPre(transform)
                : inlineSpacingPre
        const paddingAfter =
            typeof inlineSpacingAfter === 'function'
