 ...
 */
export class FlowRenderTree<
    T extends FlowNodeEntity,
> extends FlowVirtualTree<T> {
    ...
    updateRenderStruct(): void {
        ...
        // 结束节点位置更新逻辑开关
        if (
            this.document.config.get(
                FlowDocumentConfigEnum.END_NODES_REFINE_BRANCH,
            )
        ) {
    ...
    protected refineBranch(block: T) {
        ...
        while (curr) {
            if (
                ...
                const passBlocks = (children || []).filter(
                    (child) => !this.isNodeEnd(child),
                )
    ...
    protected dragNextNodesToBlock(toBlock: T, next: T) {
        ...
        const toBlockLastChild =
            toBlockInfo.children[toBlock.children.length - 1]
        ...
        if (nextInfo.parent) {
            ...
            nextParentInfo.children = nextParentInfo.children.slice(
                0,
                nextNodeIndex,
            )
