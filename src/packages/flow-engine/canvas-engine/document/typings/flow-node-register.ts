...
import { PaddingSchema } from '@flow/utils'
import { PositionSchema } from '@flow/utils'
import {
...
export interface FlowNodeMeta {
    ...
    selectable?:
        | boolean
        | ((node: FlowNodeEntity, mousePos?: PositionSchema) => boolean) // 是否可被框选
    ...
    padding?:
        | PaddingSchema
        | ((transform: FlowNodeTransformData) => PaddingSchema) // 节点设置了 padding，则不需要 inlineSpacingPre 和 inlineSpacingAfter
...
export namespace FlowNodeRegistry {
    ...
    export function extend(
        ...
        return extendRegistries.reduce(
            (res, ext) => merge(res, ext, registry.type),
            registry,
        )
    }
}
