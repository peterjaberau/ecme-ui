import { Compare, Disposable, domUtils, Emitter } from '@flow/utils'
...
export class FlowNodeRenderData extends EntityData<FlowNodeRenderSchema> {
    ...
    protected onExtInfoChangeEmitter = new Emitter<{
        newInfo: any
        oldInfo: any
    }>()
    ...
    getDefaultData(): FlowNodeRenderSchema {
        const { addable, expandable, defaultExpanded } =
    ...
    set activated(activated: boolean) {
        if (
            this.entity.flowNodeType === FlowNodeBaseType.BLOCK_ICON &&
            this.entity.parent
        ) {
            this.entity.parent.getData<FlowNodeRenderData>(
                FlowNodeRenderData,
            )!.activated = activated
            return
        }
    ...
    get activated() {
        ...
        if (
            entity.parent &&
            entity.parent.getData<FlowNodeRenderData>(FlowNodeRenderData)!
                .activated
        ) {
            return true
        }
