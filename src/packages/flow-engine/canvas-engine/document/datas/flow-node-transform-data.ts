import { Disposable, type IPoint, Rectangle } from '@flow/utils'
import {
    Bounds,
    EntityData,
    PositionSchema,
...
export class FlowNodeTransformData extends EntityData<FlowNodeTransformSchema> {
    ...
    getDefaultData(): FlowNodeTransformSchema {
        const { size, defaultExpanded, expandedSize, hidden } =
            this.entity.getNodeMeta()
    ...
    constructor(entity: FlowNodeEntity) {
        ...
        this.renderState =
            this.entity.addData<FlowNodeRenderData>(FlowNodeRenderData)
    ...
    get originDeltaX(): number {
        return this.entity.memoLocal<number>('originDeltaX', () => {
            ...
            if (getOriginDeltaX)
                return getOriginDeltaX(this, this.entity.document.layout)
            // 没有子节点，则采用自身的原点偏移
            ...
            return children.reduce((res: undefined | number, child) => {
                ...
                return res === undefined || deltaX < res
                    ? deltaX
                    : (res as number)
            }, undefined) as number
        })
    ...
    get originDeltaY(): number {
        return this.entity.memoLocal<number>('originDeltaY', () => {
            ...
            if (getOriginDeltaY)
                return getOriginDeltaY(this, this.entity.document.layout)
            // 没有子节点，则采用自身的原点偏移
            if (children.length === 0) {
            ...
            return children.reduce((res: undefined | number, child) => {
                ...
                return res === undefined || deltaY < res
                    ? deltaY
                    : (res as number)
            }, undefined) as number
        })
        ...
        return this.entity.memoGlobal<Rectangle>('bounds', () => {
            ...
            if (this.isContainer) {
                const childrenRects = transform.children.map(
                    (c) =>
                        c.entity.getData<FlowNodeTransformData>(
                            FlowNodeTransformData,
                        )!.boundsWithPadding,
                )
                // Container 要加上 inlineSpacing
                return Rectangle.enlarge(childrenRects).withPadding(
                    this.padding,
                )
            }
    ...
    get localBounds(): Rectangle {
        return this.entity.memoLocal<Rectangle>('localBounds', () => {
            ...
            if (this.isContainer) {
                ...
                const childrenBounds = Rectangle.enlarge(
                    childrenRects,
                ).withPadding(this.padding)
                return Bounds.applyMatrix(
                    childrenBounds,
    ...
    get inlineSpacingPre(): number {
        ...
        return typeof inlineSpacingPre === 'function'
            ? inlineSpacingPre(this)
            : inlineSpacingPre
    }

    get inlineSpacingAfter(): number {
        const { inlineSpacingAfter } = this.entity.getNodeMeta()
        return typeof inlineSpacingAfter === 'function'
            ? inlineSpacingAfter(this)
            : inlineSpacingAfter
    }
    ...
    get children(): FlowNodeTransformData[] {
        return this.entity.children.map(
            (child) =>
                child.getData<FlowNodeTransformData>(FlowNodeTransformData)!,
        )
    }
    ...
    get pre(): FlowNodeTransformData | undefined {
        return this.entity.pre?.getData<FlowNodeTransformData>(
            FlowNodeTransformData,
        )
    }

    get originParent(): FlowNodeTransformData | undefined {
        return this.entity.originParent?.getData<FlowNodeTransformData>(
            FlowNodeTransformData,
        )
    }
    ...
    get lastChild(): FlowNodeTransformData | undefined {
        return this.entity.lastChild?.getData<FlowNodeTransformData>(
            FlowNodeTransformData,
        )
    }

    get firstChild(): FlowNodeTransformData | undefined {
        return this.entity.firstChild?.getData<FlowNodeTransformData>(
            FlowNodeTransformData,
        )
    }
    ...
    get next(): FlowNodeTransformData | undefined {
        return this.entity.next?.getData<FlowNodeTransformData>(
            FlowNodeTransformData,
        )
    }
    ...
    get parent(): FlowNodeTransformData | undefined {
        return this.entity.parent?.getData<FlowNodeTransformData>(
            FlowNodeTransformData,
        )
    }
}
