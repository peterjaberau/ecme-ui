...
export class FlowNodeTransitionData extends EntityData<FlowNodeTransitionSchema> {
    ...
    formatLines(lines: FlowTransitionLine[]) {
        if (this.entity.document.options?.formatNodeLines) {
            return this.entity.document.options?.formatNodeLines?.(
                this.entity,
                lines,
            )
        }
    ...
    formatLabels(labels: FlowTransitionLabel[]) {
        if (this.entity.document.options.formatNodeLabels) {
            return this.entity.document.options?.formatNodeLabels?.(
                this.entity,
                labels,
            )
        }
    ...
    get lines(): FlowTransitionLine[] {
        return this.entity.memoGlobal<FlowTransitionLine[]>('lines', () => {
            const { getChildLines } =
                this.entity.parent?.getNodeRegistry() || {}

            if (getChildLines) {
                return this.formatLines(
                    getChildLines(this, this.entity.document.layout),
                )
            }
            ...
            if (getLines) {
                return this.formatLines(
                    getLines(this, this.entity.document.layout),
                )
            }
        ...
        return this.entity.memoGlobal<FlowTransitionLabel[]>('labels', () => {
            ...
            if (getChildLabels) {
                return this.formatLabels(
                    getChildLabels(this, this.entity.document.layout),
                )
            }
            ...
            if (getLabels) {
                return this.formatLabels(
                    getLabels(this, this.entity.document.layout),
                )
            }
                ...
                return this.formatLabels([
                    {
                        offset: Point.getMiddlePoint(
                            currentOutput,
                            this.transform.next.inputPoint,
                        ),
