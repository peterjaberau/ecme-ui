...
import { FlowTextKey } from '@flow/canvas-renderer'
import {
    FlowNodeBaseType,
        ...
        return [
            ...
            // 循环回撤线 - 1 分成两段可以被 viewport 识别出矩阵区域
            {
                ...
                vertices: [
                    isVertical
                        ? {
                              x: leftBlockTransform.inputPoint.x,
                              y: currentTransform.bounds.bottom,
                          }
                        : {
                              x: currentTransform.bounds.right,
                              y: leftBlockTransform.inputPoint.y,
                          },
                ],
            },
            // 循环回撤线 - 2
            {
                ...
                vertices: [
                    isVertical
                        ? {
                              x: leftBlockTransform.inputPoint.x,
                              y: currentTransform.bounds.top + 10,
                          }
                        : {
                              x: currentTransform.bounds.left + 10,
                              y: leftBlockTransform.inputPoint.y,
                          },
                ],
                arrow: true,
        ...
        if (transition.entity.originParent?.id.startsWith('while_')) {
            ...
            labels.push({
                ...
                offset: isVertical
                    ? {
                          x:
                              (currentTransform.inputPoint.x +
                                  rightBlockTransform.inputPoint.x) /
                              2,
                    ...
                    : {
                          ...
                          y:
                              (currentTransform.inputPoint.y +
                                  rightBlockTransform.inputPoint.y) /
                              2,
                      },
            })
        } else {
            ...
            labels.push({
                ...
                offset: isVertical
                    ? {
                          x: leftBlockTransform.inputPoint.x,
                          y: currentTransform.bounds.center.y + 5,
                      }
                    : {
                          x: currentTransform.bounds.center.x + 5,
                          y: leftBlockTransform.inputPoint.y,
                      },
            })
        }
