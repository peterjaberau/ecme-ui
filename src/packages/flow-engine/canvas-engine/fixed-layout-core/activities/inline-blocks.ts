...
import { FlowRendererKey } from '@flow/canvas-renderer'
import {
    DEFAULT_SPACING,
    FlowNodeBaseType,
...
export const InlineBlocksRegistry: FlowNodeRegistry = {
    ...
    meta: {
        hidden: true,
        spacing: (node) =>
            getDefaultSpacing(node.entity, ConstantKeys.NODE_SPACING),
        isInlineBlocks: true,
        inlineSpacingPre: (node) =>
            getDefaultSpacing(
                node.entity,
                ConstantKeys.INLINE_BLOCKS_PADDING_TOP,
    ...
    getChildDelta(child, layout) {
        ...
        if (preTransform) {
            ...
            if (isVertical) {
                const leftSpacing =
                    preTransform.size.width + preTransform.originDeltaX
                ...
                const bottomSpacing =
                    preTransform.size.height + preTransform.originDeltaY
    ...
    return [
        {
            ...
            props: {
                // 激活状态
                activated:
                    transition.entity.getData(FlowNodeRenderData)!.activated,
