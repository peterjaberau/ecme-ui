...
export const BlockRegistry: FlowNodeRegistry = {
    ...
    getLines(transition) {
        ...
        const hasBranchDraggingAdder =
            currentTransform &&
            currentTransform.entity.isInlineBlock &&
            transition.renderData.draggable

        // 分支拖拽场景线条 push
        // 当有其余分支的时候，绘制一条两个分支之间的线条
        if (hasBranchDraggingAdder) {
            if (isVertical) {
                const currentOffsetRightX =
                    currentTransform.firstChild?.bounds?.right || 0
                const nextOffsetLeftX =
                    currentTransform.next?.firstChild?.bounds?.left || 0
                ...
                const currentOffsetRightY =
                    currentTransform.firstChild?.bounds?.bottom || 0
                const nextOffsetLeftY =
                    currentTransform.next?.firstChild?.bounds?.top || 0
