...
export class Entity<OPTS extends EntityOpts = EntityOpts>
    implements Disposable
{
    ...
    get type(): string {
        if (!(this.constructor as any).type) {
            throw new Error(
                `Entity Registry need a type: ${this.constructor.name}`,
            )
        }
    ...
    constructor(opts: OPTS) {
        ...
        this._savedInManager =
            opts.savedInManager === undefined ? true : opts.savedInManager
    ...
    fromJSON(data?: EntityJSON | any): void {
        ...
        if (data.dataList) {
            data.dataList.forEach((d: any) => {
                const registry = this.entityManager.getDataRegistryByType(
                    d.type,
                )
                if (registry) {
    ...
    protected register(): void {
        ...
        // 注册默认 data
        this.getDefaultDataRegistries().forEach((Registry) =>
            this.addData(Registry),
        )
    }
