...
import { Disposable, DisposableCollection, Emitter } from '@flow/utils'
 ...
 */
export function bindConfigEntity(
        ...
        .toDynamicValue(
            (ctx) =>
                ctx.container
                    ...
                    .createEntity(entityRegistry)! as never,
        )
        .inSingletonScope()
}
...
export class EntityManager implements Disposable {
    ...
    init() {
        this.contributions.forEach((contrib) =>
            contrib.registerEntityManager?.(this),
        )
    }
    ...
    createEntity<T extends Entity>(
        ...
        if (!Registry.type) {
            throw new Error(
                `[EntityManager] createEntity need a type: ${Registry}`,
            )
        }
    ...
    registerEntity(Registry: EntityRegistry): void {
        if (!Registry.type)
            throw new Error(`Registry entity need a type: ${Registry.name}`)
        const oldRegistry = this.registryMap.get(Registry.type)
        if (oldRegistry) {
            if (oldRegistry !== Registry) {
                throw new Error(
                    `Entity registry ${Registry.type} need a new type`,
                )
            }
            return
        }
    ...
    registerEntityData(
        ...
        injector?: EntityDataInjector,
    ): void {
        if (!Registry.type)
            throw new Error(
                `Registry entity data need a type: ${Registry.name}`,
            )
    ...
    protected saveEntity(entity: Entity): void {
        ...
        entity
            ...
            .forEach((registry) =>
                this.fireEntityDataChanged(entity.type, registry.type),
            )
        this.fireEntityChanged(entity)
    ...
    protected removeEntity(entity: Entity): void {
        if (
            this.entityInstanceMap.has(entity.id) &&
            this.entityInstanceMapByType.has(entity.type)
        ) {
    ...
    getEntityVersion(registry: EntityRegistry | string): number {
        return (
            this.entityVersionMap.get(
                typeof registry === 'string' ? registry : registry.type,
            ) || 0
        )
    }
    ...
    getDataInjector(registry: EntityDataRegistry | string) {
        return this.dataInjectorMap.get(
            typeof registry === 'string' ? registry : registry.type,
        )
    }
