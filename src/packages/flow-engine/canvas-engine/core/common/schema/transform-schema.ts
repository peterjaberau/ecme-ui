...
export class TransformData
    extends EntityData<TransformSchema>
    implements TransformSchema
{
    ...
    constructor(entity: Entity) {
        ...
        this.bindChange(this.entity.addData(RotationData), () =>
            this.updateSkew(),
        )
    }
    ...
    get localSize(): SizeSchema {
        ...
        if (this.isContainer) {
            const childrenBounds = Rectangle.enlarge(
                this.children.map((c) => c.localBounds),
            )
            size = {
    ...
    setParent(
        parent: TransformData | undefined,
        listenParentData = true,
    ): void {
    ...
    widthToScaleX(width: number, isWorldSize?: boolean): number {
        const parentScaleX =
            isWorldSize && this.parent ? this.parent.worldScale.x : 1
    ...
    heightToScaleY(height: number, isWorldSize?: boolean): number {
        const parentScaleY =
            isWorldSize && this.parent ? this.parent.worldScale.y : 1
...
export namespace TransformData {
     ...
     * @param target
     */
    export function isParentOrChildrenTransform(
        dragableEntities: Entity[],
        target: Entity,
    ): boolean {
        ...
        for (const dragger of dragableEntities.values()) {
            const draggerTransform =
                dragger.getData<TransformData>(TransformData)
