...
import { Matrix, Rectangle } from '@flow/utils'
...
describe('bounds', () => {
    ...
    it('transform position with parent', () => {
        ...
        expect(
            Bounds.getLeftPointFromBounds(target, target.worldTransform),
        ).toEqual({
            x: -40,
            y: -40,
        })
        expect(
            Bounds.getTopPointFromBounds(target, target.worldTransform),
        ).toEqual({
            x: -40,
            y: -40,
        })
        expect(
            Bounds.getLeftPointFromBounds(parent, parent.worldTransform),
        ).toEqual({
            x: 10,
            y: 10,
        })
        expect(
            Bounds.getTopPointFromBounds(parent, parent.worldTransform),
        ).toEqual({
            x: 10,
            y: 10,
        })
