import { type Matrix, Point, Rectangle } from '@flow/utils'

import type { PositionSchema, TransformSchema } from '../schema'

type TransformOriginAndSize = Pick<TransformSchema, 'origin'> &
    Pick<TransformSchema, 'size'>
...
export namespace Bounds {
     ...
     * 位置做矩阵偏移
     */
    export function getPointWithMatrix(
    ...
    export function getLeftPointFromBounds(
        ...
        const bottomLeft = getBottomLeft(target, matrix)
        const bottomRight = getBottomRight(target, matrix)
        const items = [topLeft, topRight, bottomLeft, bottomRight].sort(
            (p1, p2) => p1.x - p2.x,
        )
        return items[0]
    }
    ...
    export function getRightCenter(
        target: TransformOriginAndSize,
        matrix?: Matrix,
    ): PositionSchema {
        const { size, origin } = target
    ...
    export function getBottomCenter(
        target: TransformOriginAndSize,
        matrix?: Matrix,
    ): PositionSchema {
        const { size, origin } = target
