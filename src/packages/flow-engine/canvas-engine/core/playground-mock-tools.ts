...
export namespace PlaygroundMockTools {
    ...
    export class LayerTestState<T extends Layer = Layer> {
        ...
        private hijackMethod(
            layer: Layer,
            layerMethod: keyof Layer & keyof LayerTestState,
        ): void {
            if (typeof layer[layerMethod] === 'function') {
                // vi should be global
                // @ts-ignore
                ;(this as any)[layerMethod] = vi.spyOn(
                    layer as any,
                    layerMethod,
                )
            }
    ...
    export function createContainer(
        ...
        container
            ...
            .toDynamicValue(
                (context) => (layerRegistry: LayerRegistry, options?: any) => {
                    ...
                    context.container
                        ...
                        .set(
                            ...
                            new LayerTestState(
                                layerInstance,
                                container.get<Playground>(Playground),
                                container,
                            ),
                        )
