...
export type PlaygroundScrollLimitFn = (scroll: {
    scrollX: number
    scrollY: number
}) => {
    scrollX: number
...
export class PlaygroundConfigEntity extends ConfigEntity<PlaygroundConfigEntityData> {
    ...
    private _onReadonlyOrDisabledChangeEmitter = new Emitter<{
        readonly: boolean
        disabled: boolean
    }>()
    ...
    readonly onReadonlyOrDisabledChange =
        this._onReadonlyOrDisabledChangeEmitter.event
    ...
    updateConfig(props: Partial<PlaygroundConfigEntityData>): void {
        ...
        if (
            props.scrollLimitX !== undefined &&
            props.scrollX! < props.scrollLimitX
        ) {
            props.scrollX = props.scrollLimitX
        }
        if (
            props.scrollLimitY !== undefined &&
        ...
        const readonlyOrDisableChanged =
            readonly !== this.readonly || disabled !== this.disabled
        if (readonlyOrDisableChanged)
            this._onReadonlyOrDisabledChangeEmitter.fire({
                readonly: this.readonly,
                disabled: this.disabled,
            })
        if (grabDisable !== this.grabDisable)
            this._onGrabDisableChangeEmitter.fire(this.grabDisable)
    }
    ...
    isViewportVisible(
        ...
        return Rectangle.isViewportVisible(
            bounds,
            this.getViewport(),
            rotation,
            includeAll,
    ...
    scrollToView(opts: PlaygroundConfigRevealOpts = {}): Promise<void> {
        ...
        if (entities && entities.length > 0) {
            const entitiesBounds = entities
                .map((e) => {
                    ...
                    const size = e.getData<SizeData>(SizeData) || {
                        width: 0,
                        height: 0,
                    }
                    if (!position) return
        ...
        const toValues = {
            scrollX:
                (bounds.x +
                    bounds.width / 2 +
                    (scrollDelta ? scrollDelta.x : 0)) *
                    scale -
                config.width / 2,
            scrollY:
                (bounds.y +
                    bounds.height / 2 +
                    (scrollDelta ? scrollDelta.y : 0)) *
                    scale -
                config.height / 2,
            zoom: opts.zoom,
        }
