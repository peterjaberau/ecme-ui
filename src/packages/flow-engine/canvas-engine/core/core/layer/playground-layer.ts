...
import { Disposable, domUtils } from '@flow/utils'
...
export class PlaygroundLayer extends Layer<PlaygroundLayerOptions> {
    ...
    onReady(): void {
        ...
        if (this.options.preventGlobalGesture) {
            ...
            this.toDispose.push(
                Disposable.create(() => gesturePreventGlobal.destroy()),
            )
        }
        this.toDispose.pushAll([
            ...
            this.listenPlaygroundEvent(
                ...
                (e: MouseEvent) => {
                    ...
                    if (
                        this.isGrab() &&
                        (this.editorStateConfig.isPressingSpaceBar ||
                            isMouseCenterButton)
                    ) {
            ...
            this.listenPlaygroundEvent(
                ...
                (e: MouseEvent) => {
                    const isSomeHovered =
                        this.options?.hoverService?.isSomeHovered()

                    // 如果是鼠标优先模式，当前位置不是节点，并且没有按下 shift，才启动拖拽
                    if (
                        this.isMouseMode() &&
                        !isSomeHovered &&
                        !this.editorStateConfig.isPressingShift
                    ) {
                        this.grabDragger.start(e.clientX, e.clientY)
                    }
                },
            ...
            this.editorStateConfig.onStateChange(
            ...
            this.listenGlobalEvent('keyup', (e: KeyboardEvent) => {
                ...
                if (
                    ...
                    this.lastShortcutState.shortcutAutoEsc
                ) {
            ...
            }),
            {
                ...
                dispose: () => {
                    ...
                    if (this.cursorStyle.parentNode) {
                        this.cursorStyle.parentNode.removeChild(
                            this.cursorStyle,
                        )
                    }
                },
    ...
    onStateChanged(e: EditorStateChangeEvent): void {
        ...
        if (state.cursor) {
            ...
            if (this.currentGesture && this.currentGesture.target.parentNode) {
                ;(
                    this.currentGesture.target.parentNode as HTMLElement
                ).style.cursor = this.getCursor(state.cursor)
            }
        } else {
    ...
    protected grabDragger = new PlaygroundDrag({
        ...
        onDrag: (e) => {
            ...
            this.config.updateConfig({
                scrollX:
                    this.startGrabScroll.scrollX - e.endPos.x + e.startPos.x,
                scrollY:
                    this.startGrabScroll.scrollY - e.endPos.y + e.startPos.y,
            })
        },
        onDragEnd: (e) => {
            if (this.isGrab()) {
    ...
    createGesture(): void {
        if (!this.currentGesture) {
            this.currentGesture = new PlaygroundGesture(
                this.pipelineNode.parentElement!,
                this.config,
            )
    ...
    protected handleWheelEvent(event: WheelEvent): void {
        ...
        if (
            (this.currentGesture && this.currentGesture.pinching) ||
            event.ctrlKey ||
            event.metaKey
        )
            return
        ...
        if (this.isMouseMode()) {
            ...
            const { zoom, minZoom, maxZoom, scrollX, scrollY } =
                this.playgroundConfigEntity.config
            ...
            // 处理横向和竖向滚轮
            const getDelta = (wheelDelta: number): number =>
                wheelDelta > 0 ? -scaleStep : scaleStep
    ...
    protected getScrollParent(ele?: HTMLElement | null): HTMLElement | null {
        ...
        const isOverflowXScrollable = ['auto', 'scroll', 'overlay'].includes(
            overflowXStyle,
        )
        const isOverflowYScrollable = ['auto', 'scroll', 'overlay'].includes(
            overflowYStyle,
        )
