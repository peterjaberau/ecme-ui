...
function getValueEvent<EventType extends TouchEvent | PointerEvent>(
    ...
    return (
        isTouch(event) ? getTouchList(event as TouchEvent)[0] : event
    ) as any
}

export function distanceAngle(
    P1: Touch | PointerEvent,
    P2: Touch | PointerEvent,
) {
    // add a try catch
...
export function scrollValues(event: UIEvent): Vector2 {
    ...
    const { scrollX, scrollY, scrollLeft, scrollTop } =
        event.currentTarget as Element & Window
