...
const KEYS_DELTA_MAP = {
    ArrowRight: (displacement: number, factor: number = 1) => [
        displacement * factor,
        0,
    ],
    ArrowLeft: (displacement: number, factor: number = 1) => [
        -1 * displacement * factor,
        0,
    ],
    ArrowUp: (displacement: number, factor: number = 1) => [
        0,
        -1 * displacement * factor,
    ],
    ArrowDown: (displacement: number, factor: number = 1) => [
        0,
        displacement * factor,
    ],
}
    ...
    setupPointer(event: PointerEvent) {
        ...
        if (process.env.NODE_ENV === 'development') {
            try {
                if (
                    device === 'pointer' &&
                    config.preventScrollDelay === undefined
                ) {
                    const currentTarget =
