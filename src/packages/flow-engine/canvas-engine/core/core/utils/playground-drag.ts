...
export class PlaygroundDrag<T = undefined> implements Disposable {
    ...
    constructor(options: PlaygroundDragOptions<T> = {}) {
        if (options.onDragStart)
            this.onDragStart((e) => options.onDragStart!(e, this.context))
        if (options.onDrag) this.onDrag((e) => options.onDrag!(e, this.context))
        if (options.onDragEnd)
            this.onDragEnd((e) => options.onDragEnd!(e, this.context))
        if (options.stopGlobalEventNames)
            this._stopGlobalEventNames = options.stopGlobalEventNames
    }
    ...
    get scale(): number {
        return this._playgroundConfigEntity
            ? this._playgroundConfigEntity.finalScale
            : 1
    }

    protected getRelativePos(event: MouseEvent): PositionSchema {
        if (this._playgroundConfigEntity) {
            return this._playgroundConfigEntity.getPosFromMouseEvent(
                event,
                false,
            )
        }
    ...
    fireScroll(scrollKey: 'scrollY' | 'scrollX', added: boolean): void {
        const current =
            scrollKey === 'scrollY'
                ? this._scrollYInterval
                : this._scrollXInterval
        if (!current) return
...
export interface PlaygroundDragEntitiesOpts<T>
    extends PlaygroundDragOptions<T> {
