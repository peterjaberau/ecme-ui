...
export function startTween<V extends TweenValues = TweenValues>(
    opts: TweenOpts<V>,
): Disposable {
    startTweenLoop()
    let stopped = false
...
export function scrollIntoViewWithTween(opts: ScrollIntoViewOpts): Disposable {
    ...
    if (startScrollTop > top) {
        ...
        if (
            startScrollTop < bottom &&
            targetNode.clientHeight < parentNode.clientHeight
        ) {
            endScrollTop = bottom
        }
    }
    if (startScrollLeft > left) {
        ...
        if (
            startScrollLeft < right &&
            targetNode.clientWidth < parentNode.clientWidth
        ) {
            endScrollLeft = right
        }
    }
