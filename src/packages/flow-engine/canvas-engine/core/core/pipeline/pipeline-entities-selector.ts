...
export class PipelineEntitiesSelector {
    ...
    getLayerEntityDatas(layer: Layer): {
        ...
        selector.datas.forEach((registries) => {
            ...
            const entityDatas = this.entityManager.getEntityDatas(
                entityRegistry,
                entityDataRegistry,
            )
            const version =
                this.entityManager.getEntityDataVersion(entityDataRegistry)
...
function checkChanged(
    v1: SelectorVersion = new Map(),
    v2: SelectorVersion = new Map(),
): boolean {
