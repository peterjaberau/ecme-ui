...
export class PipelineRegistry implements Disposable, IMessageHandler {
    ...
    readonly onScrollEmitter = new Emitter<{
        scrollX: number
        scrollY: number
    }>()
    ...
    registerLayer<P extends Layer = Layer>(
        ...
        if (layer.onReadonlyOrDisabledChange) {
            this.configEntity.onReadonlyOrDisabledChange(
                layer.onReadonlyOrDisabledChange.bind(layer),
            )
        }
    ...
    ready(): void {
        ...
        config.onConfigChanged(() => {
            ...
            if (
                lastScroll.scrollX !== newScroll.scrollX ||
                lastScroll.scrollY !== newScroll.scrollY
            ) {
