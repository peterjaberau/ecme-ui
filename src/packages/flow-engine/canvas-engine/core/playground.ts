...
import { CommandService } from '@flow/command'

import { SelectionService } from './services'
import {
    PlaygroundContribution,
    PlaygroundRegistry,
} from './playground-contribution'
...
export class Playground<CONTEXT = PlaygroundContext> implements Disposable {
    ...
    constructor(
        ...
        const editStates =
            this.entityManager.createEntity<EditorStateConfigEntity>(
                EditorStateConfigEntity,
            )
        ...
        if (playgroundConfig.editorStates)
            playgroundConfig.editorStates.forEach((state) =>
                editStates.registerState(state),
            )
        if (playgroundConfig.zoomEnable !== undefined)
            this.zoomEnable = playgroundConfig.zoomEnable
        if (playgroundConfig.entityConfigs) {
            for (const [k, v] of playgroundConfig.entityConfigs) {
                const entity = this.entityManager.getEntity<ConfigEntity>(
                    k,
                    true,
                )
    ...
    init(): void {
        ...
        for (const contrib of contributions) {
            if (contrib.registerPlayground)
                contrib.registerPlayground(this.registry)
        }
        for (const contrib of contributions) {
    ...
    registerLayers(...layerRegistries: LayerRegistry[]): void {
        layerRegistries.forEach((layer) =>
            this.pipelineRegistry.registerLayer(layer),
        )
    }
