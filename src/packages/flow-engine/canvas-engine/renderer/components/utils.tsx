    ...
    if (isVertical) {
        const nextWidth =
            data.entity.next?.firstChild && !data.entity.next.isInlineBlocks
                ? data.entity.next.firstChild!.getData(FlowNodeTransformData)!
                      .size.width
...
export function getTransitionLabelHoverHeight(data: FlowNodeTransitionData) {
    ...
    if (isVertical) {
        if (data.transform.next) {
            return (
                data.transform.next.inputPoint.y - data.transform.outputPoint.y
            )
        }
        return DEFAULT_LABEL_ACTIVATE_HEIGHT
    }
    const nextHeight =
        data.entity.next?.firstChild && !data.entity.next.isInlineBlocks
            ? data.entity.next.firstChild!.getData(FlowNodeTransformData)!.size
                  .height
