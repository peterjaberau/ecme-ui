        ...
        () =>
            realVertices
                .map((point, idx) => {
                    ...
                    // 前后 delta 变化
                    const prevDelta = {
                        x: Math.abs(prev.x - point.x),
                        y: Math.abs(prev.y - point.y),
                    }
                    const nextDelta = {
                        x: Math.abs(next.x - point.x),
                        y: Math.abs(next.y - point.y),
                    }
                    ...
                    if (!isRightAngle) {
                        console.error(
                            `vertex ${point.x},${point.y} is not right angle`,
                        )
                    }
                    ...
                    const radiusX = isNil(point.radiusX)
                        ? radius
                        : point.radiusX
                    const radiusY = isNil(point.radiusY)
                        ? radius
                        : point.radiusY
                    let rx = radiusX
                    let ry = radiusY
