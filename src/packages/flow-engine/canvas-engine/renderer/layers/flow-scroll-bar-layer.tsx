...
import { domUtils, Rectangle } from '@flow/utils'
// import {
//   FlowDocument,
...
export class FlowScrollBarLayer extends Layer<ScrollBarOptions> {
    ...
    // 右滚动区域
    readonly rightScrollBar = domUtils.createDivWithClass(
        'gedit-playground-scroll-right',
    )

    // 右滚动条
    readonly rightScrollBarBlock = domUtils.createDivWithClass(
        'gedit-playground-scroll-right-block',
    )

    // 底滚动区域
    readonly bottomScrollBar = domUtils.createDivWithClass(
    ...
    getToLeft(scrollX: number): number {
        return (
            ((scrollX - this.mostRight) / this.viewportMoveWidth) *
            this.clientViewportWidth
        )
    }

    getToTop(scrollY: number): number {
        return (
            ((scrollY - this.mostBottom) / this.viewportMoveHeight) *
            this.clientViewportHeight
        )
    }
    ...
    clickRightScrollBar(e: MouseEvent) {
        ...
        const scrollY =
            (this.mostTop - this.viewportFullHeight * ratio) * this.scale
    ...
    clickBottomScrollBar(e: MouseEvent) {
        ...
        const scrollX =
            (this.mostLeft - this.viewportFullWidth * ratio) * this.scale
    ...
    protected changeScrollBarVisibility(
        scrollBar: HTMLDivElement,
        status: ScrollBarVisibility,
    ) {
        const addClassName =
    ...
    onReady() {
        ...
        if (this.options.showScrollBars === 'whenScrolling') {
            this.rightScrollBar.addEventListener(
                'mouseenter',
                (e: MouseEvent) => {
                    this.changeScrollBarVisibility(
                        this.rightScrollBarBlock,
        ...
        // 监听拖拽滚动
        this.bottomScrollBarBlock.addEventListener(
            'mousedown',
            (e: MouseEvent) => {
        ...
        this.rightScrollBarBlock.addEventListener(
            'mousedown',
            (e: MouseEvent) => {
    ...
    autorun() {
        ...
        const paddingLeftRight =
            (this.viewportWidth - viewportBounds.width) / 2 - BORDER_WIDTH
        // 中间部分元素的上下间距
        const paddingTopBottom =
            (this.viewportHeight - viewportBounds.height) / 2 - BORDER_WIDTH
        ...
        this.scrollBottomWidth =
            calcViewportWidth -
            (calcViewportWidth * (this.mostLeft - this.mostRight)) /
                this.viewportMoveWidth
        // 右侧滚动条高度
        this.scrollRightHeight =
            calcViewportHeight -
            (calcViewportHeight * (this.mostTop - this.mostBottom)) /
                this.viewportMoveHeight
