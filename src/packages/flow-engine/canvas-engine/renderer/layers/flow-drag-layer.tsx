...
import { PlaygroundDrag } from '@flow/canvas-core'
...
import {
    type CollisionRetType,
    FlowDragEntity,
} from '../entities/flow-drag-entity'
...
export class FlowDragLayer extends Layer<FlowDragOptions> {
    ...
    handleMouseMove(event: MouseEvent) {
        if (this.dragStartEntity && this.dragEnable(event)) {
            ...
            if (this.containerRef.current) {
                ...
                const collisionTransition = this.transitions.find(
                    (transition) => {
                        // 过滤已被折叠 label
                        if (transition?.entity?.parent?.collapsed) {
                            return false
                        }
                        const { hasCollision, labelOffsetType } =
                            this.flowDragConfigEntity.isCollision(
                                transition,
                                draggingRect,
                                isBranch,
                            ) as CollisionRetType
                ...
                if (
                    collisionTransition &&
                    (isBranch
                        ? this.service.isDroppableBranch(
                              collisionTransition.entity,
                              side,
                          )
                        : this.service.isDroppableNode(
                              collisionTransition.entity,
                          )) &&
                ...
                this.pipelineNode.parentElement!.appendChild(
                    this.draggingNodeMask,
                )
    ...
    handleMouseUp() {
        ...
        if (this.containerRef.current) {
            ...
            if (
                this.pipelineNode.parentElement!.contains(this.draggingNodeMask)
            ) {
                this.pipelineNode.parentElement!.removeChild(
                    this.draggingNodeMask,
                )
            }
        }
    ...
    async startDrag(
        ...
        const isIcon =
            startEntityFromProps.flowNodeType === FlowNodeBaseType.BLOCK_ICON
        const isOrderIcon =
            startEntityFromProps.flowNodeType ===
            FlowNodeBaseType.BLOCK_ORDER_ICON

        const dragStartEntity =
            isIcon || isOrderIcon
                ? startEntityFromProps.parent!
                : startEntityFromProps
    ...
    onReady() {
        ...
        this.dragNodeComp = this.rendererRegistry.getRendererComponent(
