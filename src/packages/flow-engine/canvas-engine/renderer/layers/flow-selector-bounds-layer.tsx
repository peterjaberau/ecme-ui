...
export class FlowSelectorBoundsLayer extends Layer<FlowSelectorBoundsLayerOptions> {
    ...
    readonly selectBoundsBackground = domUtils.createDivWithClass(
    ...
    onReady(): void {
        ...
        if (this.options.boundsPadding !== undefined) {
            this.flowSelectConfigEntity.boundsPadding =
                this.options.boundsPadding
        }
        if (this.options.backgroundClassName) {
            this.selectBoundsBackground.classList.add(
                this.options.backgroundClassName,
            )
        }
        // 这里创建一个空 layer 用于放背景
    ...
    render(): JSX.Element {
        ...
        const SelectorBoxPopover =
            ...
            this.rendererRegistry.tryToGetRendererComponent(
                FlowRendererKey.SELECTOR_BOX_POPOVER,
            )?.renderer
