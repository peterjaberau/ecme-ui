...
import { type PlaygroundConfigEntity } from '@flow/canvas-core'
...
export function scrollLimit(
    ...
    const oldScroll = {
        scrollX: configData.scrollX,
        scrollY: configData.scrollY,
    }
    // 画布 size 还没初始化滚动不限制
    if (
        boundsList.length === 0 ||
    ...
    const isVisible = boundsList.find((bounds) =>
        Rectangle.isViewportVisible(bounds, viewport),
    )
    if (!isVisible) {
