...
export class FlowDragEntity extends ConfigEntity<FlowDragEntityConfig> {
    ...
    isBranchCollision(
        ...
        const hasCollision = labels.some((label) => {
            if (
                !label ||
                label.type !== FlowTransitionLabelEnum.BRANCH_DRAGGING_LABEL
            ) {
                return false
            }
            const hoverHeight = isVertical
    ...
    scrollDirection(
        ...
        const mouseToBottom =
            playgroundConfig.height + playgroundConfig.clientY - e.clientY
        ...
        this._stopScrollY()
        const mouseToRight =
            playgroundConfig.width + playgroundConfig.clientX - e.clientX
