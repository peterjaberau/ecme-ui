...
import { getNodeForm } from '@flow/node-engine'
...
export function useNodeRender(
    nodeFromProps?: WorkflowNodeEntity,
): NodeRenderReturnType {
    const node =
        nodeFromProps || useContext<WorkflowNodeEntity>(PlaygroundEntityContext)
    ...
    useEffect(() => {
        const disposable = dragService.onDragLineEventChange(
            ({ type, onDragNodeId }) => {
                if (type === 'onDrag') {
    ...
    const startDrag = useCallback(
        (e: React.MouseEvent) => {
            ...
            // 输入框不能拖拽
            if (
                !checkTargetDraggable(e.target) ||
                !checkTargetDraggable(document.activeElement)
            ) {
                return
            }
