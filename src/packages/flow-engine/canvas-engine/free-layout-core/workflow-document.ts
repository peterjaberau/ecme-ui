...
import { Disposable, Emitter, type IPoint } from '@flow/utils'
import { NodeEngineContext } from '@flow/form-core'
import {
    FlowDocument,
    FlowNodeBaseType,
...
export class WorkflowDocument extends FlowDocument {
    ...
    createWorkflowNode(
        ...
        if (subCanvas) {
            const canvasTransform =
                subCanvas.canvasNode.getData<TransformData>(TransformData)
    ...
    createWorkflowNodeByType(
        ...
        if (id === undefined) {
            ...
            if (this.entityManager.getEntityById(id)) {
                throw new Error(
                    `[WorkflowDocument.createWorkflowNodeByType] Node Id "${id}" duplicated.`,
                )
            }
        }
    ...
    getAssociatedNodes(): WorkflowNodeEntity[] {
        ...
        const nodeInContainer = allNode
            .filter(
                (node) =>
                    node.parent?.getNodeMeta<WorkflowNodeMeta>().isContainer,
            )
            .map((node) => node.id)
        ...
        const associatedNodes = allNode.filter((node) =>
            associatedCache.has(node.id),
        )
    ...
    fireContentChange(event: WorkflowContentChangeEvent): void {
        if (
            this._loading ||
            this.disposed ||
            this.entityManager.changeEntityLocked
        ) {
            return
        }
    ...
    private toNodeJSONFromOptions(node: WorkflowNodeEntity): WorkflowNodeJSON {
        ...
        return WorkflowDocumentOptionsDefault.toNodeJSON!(
            node,
        ) as WorkflowNodeJSON
    }
    ...
    canRemove(node: WorkflowNodeEntity, silent?: boolean): boolean {
        ...
        if (
            this.options.canDeleteNode &&
            !this.options.canDeleteNode(node, silent)
        ) {
            return false
        }
        return true
    ...
    private flatJSON(json: Partial<WorkflowJSON> = { nodes: [], edges: [] }): {
        ...
        const rootEdgeIDs: string[] = rootEdges.map((edge) =>
            this.getEdgeID(edge),
        )
    ...
    private toLineJSON(line: WorkflowLineEntity): WorkflowEdgeJSON | undefined {
        ...
        if (
            fromSubCanvas &&
            !fromSubCanvas.isCanvas &&
            toSubCanvas &&
            toSubCanvas.isCanvas
        ) {
            // 忽略子画布与父节点的连线
            return
        }
