...
import { FlowDocument, FlowDocumentContribution } from '@flow/canvas-document'
...
export const WorkflowDocumentContainerModule = new ContainerModule(
    (bind, unbind, isBound, rebind) => {
        ...
        bind(WorkflowOperationBaseService)
            .to(WorkflowOperationBaseServiceImpl)
            .inSingletonScope()
        bind(URLParams)
            .toDynamicValue(() => getUrlParams())
            .inSingletonScope()
        bindContributions(bind, WorkflowDocumentContribution, [
