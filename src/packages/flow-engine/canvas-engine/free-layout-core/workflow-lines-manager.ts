...
export class WorkflowLinesManager {
    ...
    protected onAvailableLinesChangeEmitter =
        new Emitter<WorkflowContentChangeEvent>()
    ...
    getCloseInLineFromMousePos(
        ...
        let targetLine: WorkflowLineEntity | undefined,
            targetLineDist: number | undefined
        this.getAllLines().forEach((line) => {
            const dist = line.getHoverDist(mousePos)

            if (
                dist <= minDistance &&
                (!targetLineDist || targetLineDist >= dist)
            ) {
                targetLineDist = dist
                targetLine = line
            }
