...
export class WorkflowDragService {
    ...
    init() {
        this._toDispose.pushAll([
            this._onDragLineEventEmitter,
            this._nodesDragEmitter,
        ])
    ...
    startDragSelectedNodes(
        ...
        const dragger = new PlaygroundDrag({
            ...
            onDrag: (dragEvent) => {
                if (
                    !dragSuccess &&
                    checkDragSuccess(Date.now() - startTime, dragEvent)
                ) {
                    dragSuccess = true
                }
    ...
    async startDragCard(
        ...
        const dragger = new PlaygroundDrag({
            onDragStart: (e) => {
                ...
                domNode = cloneNode
                    ? cloneNode(e)
                    : (targetNode.cloneNode(true) as HTMLDivElement)
            ...
            onDrag: (e) => {
                ...
                const collisionTransform = this._droppableTransforms.find(
                    (transform) => {
            ...
            onDragEnd: async (e) => {
                ...
                const dragNode = allowDrop
                    ? await this.dropCard(type, e, data, dropNode)
                    : undefined
                this.clearDrop()
                if (dragNode) {
                    domNode.remove()
    ...
    public adjustSubNodePosition(
        ...
        if (
            !subNodeType ||
            !containerNode ||
            containerNode.flowNodeType === FlowNodeBaseType.ROOT
        ) {
            return mousePos
        }
        const isParentEmpty =
            !containerNode.children || containerNode.children.length === 0
        const parentPadding = this.document.layout.getPadding(containerNode)
        const parentTransform =
            containerNode.getData<TransformData>(TransformData)
    ...
    private getDragPosOffset(params: {
        ...
        const adjustedOffsets: IPoint[] = Array.from(
            this.posAdjusters.values(),
        ).map((adjuster) =>
            adjuster({
                selectedNodes,
        ...
        this._droppableTransforms = this.document
            ...
            .filter((transform) => {
                ...
                if (entity.originParent) {
                    return (
                        this.nodeSelectable(entity) &&
                        this.nodeSelectable(entity.originParent)
                    )
                }
        ...
        if (
            ...
            (originLine?.toPort === toPort ||
                (toPort.portType === 'input' &&
                    this.linesManager.canAddLine(fromPort, toPort, true)))
        ) {
    ...
    private childrenOfContainer(
        ...
        if (
            !sourceContainer ||
            sourceContainer.collapsedChildren.length !== nodes.length
        ) {
            return
        }
    ...
    async startDrawingLine(
        ...
        const isFromInActivePort =
            !originLine && fromPort.isErrorPort() && fromPort.disabled
        if (
            originLine?.disabled ||
            isFromInActivePort ||
            this.playgroundConfig.readonly ||
        ...
        const dragger = new PlaygroundDrag({
            onDrag: (e) => {
                if (
                    !line &&
                    checkDragSuccess(Date.now() - startTime, e, originLine)
                ) {
                    // 隐藏原来的线条
                    if (originLine) {
                        originLine.highlightColor =
                            this.linesManager.lineColor.hidden
                    }
                    dragSuccess = true
                    // 创建临时的线条
                ...
                if (toNode && !this.isContainer(toNode)) {
                    ...
                    const { hasError } = this.handleDragOnNode(
                        toNode,
                        fromPort,
                        line,
                        toPort,
            ...
            onDragEnd: async (e) => {
                const dragPos = config.getPosFromMouseEvent(e)
                const onDragLineEndCallbacks = Array.from(
                    this._onDragLineEndCallbacks.values(),
                )
                ...
                if (dragSuccess) {
                    ...
                    if (
                        !toPort ||
                        !this.linesManager.canAddLine(fromPort, toPort, false)
                    ) {
                        // 无法添加成功
                        return end()
                    }
                    const newLine = this.linesManager.createLine(
                        newLineInfo as WorkflowLinePortInfo,
                    )
                    if (!newLine) {
                        end()
                    }
