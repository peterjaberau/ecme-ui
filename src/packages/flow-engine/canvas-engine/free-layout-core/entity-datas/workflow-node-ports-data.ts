...
export class WorkflowNodePortsData extends EntityData {
        ...
        if (meta.useDynamicPort) {
            this.toDispose.push(
                ...
                entity.getData!(SizeData)!.onDataChange(() => {
                    // 有可能节点被销毁了
                    if (
                        entity.getData!(SizeData).width &&
                        entity.getData!(SizeData).height
                    ) {
    ...
    public updateDynamicPorts(): void {
        ...
        const elements =
            domNode.querySelectorAll<HTMLDivElement>('[data-port-id]')
    ...
    public updatePorts(ports: WorkflowPorts): void {
        if (!isEqual(this._prePorts, ports)) {
            const portKeys = ports.map((port) =>
                this.getPortId(port.type, port.portID),
            )
    ...
    protected getPortEntity(portId: string): WorkflowPortEntity | undefined {
        ...
        return this.entity.entityManager.getEntityById<WorkflowPortEntity>(
            portId,
        )!
    }
    ...
    protected createPortEntity(portInfo: WorkflowPort): WorkflowPortEntity {
        ...
        let portEntity =
            this.entity.entityManager.getEntityById<WorkflowPortEntity>(id)
        if (!portEntity) {
            portEntity =
                this.entity.entityManager.createEntity<WorkflowPortEntity>(
                    WorkflowPortEntity,
                    {
                        id,
                        node: this.entity,
     ...
     * 获取或创建 port 实体
     */
    protected getOrCreatePortEntity(
        portInfo: WorkflowPort,
    ): WorkflowPortEntity {
        const id = this.getPortId(portInfo.type, portInfo.portID)
