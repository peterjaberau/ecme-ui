import { VariableEngine } from '@flow/variable-core'
import { type ASTNode, ASTNodeJSON } from '@flow/variable-core'
...
export class FlowNodeVariableData extends EntityData {
    ...
    initPrivate(): FlowNodeScope {
        if (!this._private) {
            this._private = this.variableEngine.createScope(
                `${this.entity.id}_private`,
                {
                    node: this.entity,
