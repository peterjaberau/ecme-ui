...
import { Scope, ScopeChain } from '@flow/variable-core'
import { FlowDocument, type FlowVirtualTree } from '@flow/canvas-document'
...
export class FixedLayoutScopeChain extends ScopeChain {
    ...
    getDeps(scope: FlowNodeScope): FlowNodeScope[] {
        ...
        while (curr) {
            ...
            if (curr === node) {
                // public 可以依赖 private
                if (
                    scope.meta.type === FlowNodeScopeTypeEnum.public &&
                    currData?.private
                ) {
                    deps.unshift(currData.private)
                }
            } else if (
                this.hasChildren(curr) &&
                !this.isNodeChildrenPrivate(curr)
            ) {
            ...
            if (parent) {
                ...
                let currParentPre: ScopeChainNode | undefined =
                    this.tree.getPre(currParent)
    ...
    getCovers(scope: FlowNodeScope): FlowNodeScope[] {
        ...
        while (curr) {
            ...
            if (parent) {
                let currParent: ScopeChainNode | undefined = parent
                let currParentNext: ScopeChainNode | undefined =
                    this.tree.getNext(currParent)
    ...
    sortAll(): Scope[] {
        ...
        const startNode = startNodeId
            ? this.flowDocument?.getNode(startNodeId)
            : undefined
        if (!startNode) {
            return []
        }
        ...
        return [
            startVariableData.public,
            ...this.getCovers(startVariableData.public),
        ]
    }
    ...
    private getVariableData(
        node: ScopeChainNode,
    ): FlowNodeVariableData | undefined {
    ...
    private hasChildren(node?: ScopeChainNode): boolean {
        return Boolean(
            this.tree && node && this.tree.getChildren(node).length > 0,
        )
    }
    ...
    private getAllSortedChildScope(
        node: ScopeChainNode,
        {
            ...
            ignoreNodeChildrenPrivate?: boolean
            addNodePrivateScope?: boolean
        } = {},
    ): FlowNodeScope[] {
        ...
        scopes.push(
            ...children
                .map((child) =>
                    this.getAllSortedChildScope(child, {
                        ignoreNodeChildrenPrivate,
                        addNodePrivateScope,
                    }),
                )
