...
export class ScopeEventData {
    event$: Subject<GlobalEventActionType> =
        new Subject<GlobalEventActionType>()

    dispatch<ActionType extends GlobalEventActionType = GlobalEventActionType>(
        action: ActionType,
    ) {
    ...
    on<ActionType extends GlobalEventActionType = GlobalEventActionType>(
        ...
        return subsToDisposable(
            this.event$
                .pipe(filter((_action) => _action.type === type))
                .subscribe(observer as Observer),
        )
    }
