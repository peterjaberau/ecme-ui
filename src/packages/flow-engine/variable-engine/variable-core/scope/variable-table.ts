...
export class VariableTable implements Disposable {
    ...
    protected variables$: Subject<VariableDeclaration[]> = new Subject<
        VariableDeclaration[]
    >()
     ...
     */
    onAnyVariableChange(
        observer: (changedVariable: VariableDeclaration) => void,
    ) {
    ...
    onAnyChange(observer: () => void) {
        ...
        disposables.pushAll([
            this.onDataChange(observer),
            this.onAnyVariableChange(observer),
        ])
        return disposables
    }
    ...
    getByKeyPath(keyPath: string[]): BaseVariableField | undefined {
        ...
        return propertyKeys.length
            ? variable?.getByKeyPath(propertyKeys)
            : variable
    }
    ...
    dispose(): void {
        this.variableKeys.forEach((_key) =>
            this.parentTable?.removeVariableFromTable(_key),
        )
