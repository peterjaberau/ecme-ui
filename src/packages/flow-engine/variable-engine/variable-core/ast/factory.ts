...
export namespace ASTFactory {
    ...
    export const createProperty = <VariableMeta = any>(
        json: PropertyJSON<VariableMeta>,
    ) => ({
        kind: ASTKind.Property,
        ...json,
    })
    export const createVariableDeclarationList = (
        json: VariableDeclarationListJSON,
    ) => ({
     ...
     * 表达式相关
     */
    export const createEnumerateExpression = (
        json: EnumerateExpressionJSON,
    ) => ({
