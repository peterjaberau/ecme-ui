...
export class ObjectType extends BaseType<ObjectJSON> {
    ...
    getByKeyPath(keyPath: string[]): Property | undefined {
        ...
        if (
            property?.type &&
            property?.type?.flags & ASTNodeFlags.DrilldownType
        ) {
            return property.type.getByKeyPath(restKeyPath) as
                | Property
                | undefined
        }
    ...
    protected customStrongEqual(targetTypeJSON: ASTNodeJSON): boolean {
        ...
        const targetPropertyKeys = targetProperties.map(
            (_target) => _target.key,
        )

        const isKeyStrongEqual = !xor(sourcePropertyKeys, targetPropertyKeys)
            .length

        return (
            isKeyStrongEqual &&
            targetProperties.every((targetProperty) => {
                const sourceProperty = this.propertyTable.get(
                    targetProperty.key,
                )
