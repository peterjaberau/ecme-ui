...
import { Disposable, DisposableCollection } from '@flow/utils'
...
export interface ASTNodeRegistry<
    ...
    implements Disposable
{
     ...
     */
    public readonly value$: BehaviorSubject<ASTNode> =
        new BehaviorSubject<ASTNode>(this as ASTNode)
    ...
    get kind(): string {
        if (!(this.constructor as any).kind) {
            throw new Error(
                `ASTNode Registry need a kind: ${this.constructor.name}`,
            )
        }
    ...
    toJSON(): ASTNodeJSON {
        ...
        console.warn(
            '[VariableEngine] Please Implement toJSON method for ' + this.kind,
        )
     ...
     * @returns
     */
    protected createChildNode<ChildNode extends ASTNode = ASTNode>(
        json: ASTNodeJSON,
    ): ChildNode {
     ...
     */
    protected updateChildNodeByKey(
        keyInThis: keyof this,
    ...
    dispatchGlobalEvent<
        ActionType extends GlobalEventActionType = GlobalEventActionType,
