...
export class HistoryDatabase extends Dexie {
    ...
    addHistoryRecord(
        ...
        operations: HistoryOperationRecord[],
    ) {
        return this.transaction(
            ...
            this.operation,
            async () => {
                const count = await this.history
                ...
                return Promise.all([
                    ...
                    this.operation.bulkAdd(operations),
                ])
            },
        )
    }
     ...
     * @returns
     */
    async updateHistoryByUUID(
        uuid: string,
        historyRecord: Partial<HistoryRecord>,
    ) {
            ...
            async () => {
                await Promise.all(
                    this.tables.map((table) =>
                        table.where({ resourceURI }).delete(),
                    ),
                )
            },
        )
