...
const toStyleKey = (key: string) =>
    key.replace(/([A-Z])/, (k) => `-${k.toLowerCase()}`)
...
export namespace domUtils {
    ...
    export function translatePercent(
        node: HTMLDivElement,
        x: number,
        y: number,
    ): void {
    ...
    export function classNameWithPrefix(
        prefix: string,
        ...
        return Cache.create<T>((/* item */) => {
            ...
            const dom =
                typeof className === 'string'
                    ? domUtils.createDivWithClass(className)
                    : className()
            if (children) {
