...
describe('Rectangle', () => {
    describe('Rectangle Class', () => {
        ...
        test('containsRectangle', async () => {
            ...
            expect(
                new R(1, 2, 3, 4).containsRectangle(new R(1, 2, 3, 4)),
            ).toEqual(true)
            expect(
                new R(0, 0, 2, 2).containsRectangle(new R(1, 1, 1, 1)),
            ).toEqual(true)
            expect(
                new R(0, 0, 2, 2).containsRectangle(new R(1, 1, 2, 2)),
            ).toEqual(false)
        })
        ...
        test('fit', async () => {
            expect(new R(0, 0, 2, 2).fit(new R(0, 0, 1, 1))).toEqual(
                new R(0, 0, 1, 1),
            )
            expect(new R(0, 0, 2, 2).fit(new R(1, 1, 2, 2))).toEqual(
                new R(1, 1, 1, 1),
            )
            expect(new R(0, 0, 2, 2).fit(new R(3, 3, 1, 1))).toEqual(
                new R(3, 3, 0, 0),
            )
        })
        ...
        test('enlarge', async () => {
            ...
            expect(new R().enlarge(new R(0, 0, 1, 1))).toEqual(
                new R(0, 0, 1, 1),
            )
            expect(new R(0, 0, 1, 1).enlarge(new R(1, 1, 1, 1))).toEqual(
                new R(0, 0, 2, 2),
            )
            expect(new R(0, 0, 2, 2).enlarge(new R(1, 1, 2, 2))).toEqual(
                new R(0, 0, 3, 3),
            )
        })
        ...
        test('withPadding', async () => {
            expect(
                new R(1, 1, 4, 4).withPadding({
                    left: 1,
                    right: 1,
                    top: 1,
                    bottom: 1,
                }),
            ).toEqual(new R(0, 0, 6, 6))
        })
    ...
    test('align', async () => {
        ...
        expect(
            R.align([r1.clone(), r2.clone()], RectangleAlignType.ALIGN_BOTTOM),
        ).toEqual([new R(0, 2, 1, 1), new R(1, 1, 2, 2)])
        expect(
            R.align([r1.clone(), r2.clone()], RectangleAlignType.ALIGN_CENTER),
        ).toEqual([new R(1, 0, 1, 1), new R(0.5, 1, 2, 2)])
        expect(
            R.align([r1.clone(), r2.clone()], RectangleAlignType.ALIGN_LEFT),
        ).toEqual([new R(0, 0, 1, 1), new R(0, 1, 2, 2)])
        expect(
            R.align([r1.clone(), r2.clone()], RectangleAlignType.ALIGN_MIDDLE),
        ).toEqual([new R(0, 1, 1, 1), new R(1, 0.5, 2, 2)])
        expect(
            R.align([r1.clone(), r2.clone()], RectangleAlignType.ALIGN_RIGHT),
        ).toEqual([new R(2, 0, 1, 1), new R(1, 1, 2, 2)])
        expect(
            R.align([r1.clone(), r2.clone()], RectangleAlignType.ALIGN_TOP),
        ).toEqual([new R(0, 0, 1, 1), new R(1, 0, 2, 2)])
        ...
        expect(
            R.align([r1.clone()], RectangleAlignType.DISTRIBUTE_VERTICAL),
        ).toEqual([r1])
        expect(R.align([r1.clone(), r2.clone()], '' as any)).toEqual([r1, r2]) // override default
    })

    test('enlarge', async () => {
        expect(R.enlarge([new R(0, 0, 1, 1), new R(1, 1, 1, 1)])).toEqual(
            new R(0, 0, 2, 2),
        )
    ...
    test('intersects', async () => {
        ...
        expect(
            R.intersects(new R(0, 0, 2, 2), new R(1, 1, 2, 2), 'horizontal'),
        ).toEqual(true)
        expect(
            R.intersects(new R(0, 0, 2, 2), new R(1, 1, 2, 2), 'vertical'),
        ).toEqual(true)

        expect(R.intersects(new R(0, 0, 2, 2), new R(3, 3, 2, 2))).toEqual(
            false,
        )
        expect(
            R.intersects(new R(0, 0, 2, 2), new R(3, 3, 2, 2), 'horizontal'),
        ).toEqual(false)
        expect(
            R.intersects(new R(0, 0, 2, 2), new R(3, 3, 2, 2), 'vertical'),
        ).toEqual(false)
    })

    test('OBBRect', async () => {
        expect(
            new OBBRect(new Point(0, 0), 1, 1, 0).getProjectionRadius(
                new Vector2(0, 1),
            ),
        ).toBeCloseTo(0.5)
        expect(
        ...
        expect(
            new OBBRect(new Point(0, 0), 1, 1, PI / 4).getProjectionRadius(
                new Vector2(1, 1),
            ),
        ).toBeCloseTo(Math.sqrt(2) / 2)
        expect(
            new OBBRect(new Point(0, 0), 1, 1, PI / 2).getProjectionRadius(
                new Vector2(1, 1),
            ),
        ).toEqual(1)
    })
    ...
    test('intersectsWithRotation', async () => {
        expect(R.intersects(new R(0, 0, 10, 10), new R(8, 8, 4, 4))).toEqual(
            true,
        )
        expect(new R(0, 0, 10, 10).fit(new R(8, 8, 4, 4))).toEqual(
            new R(8, 8, 2, 2),
        )
        expect(
    ...
    test('isViewportVisible', async () => {
        expect(
            R.isViewportVisible(new R(0, 0, 1, 1), new R(0.5, 0.5, 1, 1)),
        ).toEqual(true)
        expect(
            R.isViewportVisible(
                new R(0, 0, 1, 1),
    ...
    test('createRectangleWithTwoPoints', async () => {
        expect(
            R.createRectangleWithTwoPoints({ x: 0, y: 0 }, { x: 1, y: 1 }),
        ).toEqual(new R(0, 0, 1, 1))
        expect(
            R.createRectangleWithTwoPoints({ x: 1, y: 1 }, { x: 0, y: 0 }),
        ).toEqual(new R(0, 0, 1, 1))
    })
