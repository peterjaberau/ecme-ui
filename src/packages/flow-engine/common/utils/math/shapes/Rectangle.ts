...
export class Rectangle {
    ...
    ceil(resolution = 1, precision = 0.001): this {
        const x2 =
            Math.ceil((this.x + this.width - precision) * resolution) /
            resolution
        const y2 =
            Math.ceil((this.y + this.height - precision) * resolution) /
            resolution
...
export namespace Rectangle {
    ...
    export function align(
        ...
        switch (type) {
            ...
            case RectangleAlignType.DISTRIBUTE_HORIZONTAL:
                ...
                const sort = rectangles
                    .slice()
                    .sort((r1, r2) => r1.left - r2.left)
            ...
            case RectangleAlignType.DISTRIBUTE_VERTICAL:
                if (rectangles.length <= 2) break
                const sort2 = rectangles
                    .slice()
                    .sort((r1, r2) => r1.top - r2.top)
                const bounds2 = enlarge(rectangles)
                const space2 =
                    rectangles.reduce(
                        (s, rect) => s - rect.height,
                        bounds2.height,
                    ) /
                    (rectangles.length - 1)
    ...
    export function intersectsWithRotation(
        ...
        const obb1 = new OBBRect(
            rect1.center,
            rect1.width,
            rect1.height,
            rotate1,
        )
        const obb2 = new OBBRect(
            rect2.center,
            rect2.width,
            rect2.height,
            rotate2,
        ...
        if (
            obb1.getProjectionRadius(axisA1) +
                obb2.getProjectionRadius(axisA1) <=
            Math.abs(nv.dot(axisA1))
        )
            return false
        const axisA2 = obb1.axesY
        if (
            obb1.getProjectionRadius(axisA2) +
                obb2.getProjectionRadius(axisA2) <=
            Math.abs(nv.dot(axisA2))
        )
            return false
        const axisB1 = obb2.axesX
        if (
            obb1.getProjectionRadius(axisB1) +
                obb2.getProjectionRadius(axisB1) <=
            Math.abs(nv.dot(axisB1))
        )
            return false
        const axisB2 = obb2.axesY
        if (
            obb1.getProjectionRadius(axisB2) +
                obb2.getProjectionRadius(axisB2) <=
     ...
     * 根据两点创建矩形
     */
    export function createRectangleWithTwoPoints(
        point1: IPoint,
        point2: IPoint,
    ): Rectangle {
