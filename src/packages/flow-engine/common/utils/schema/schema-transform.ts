...
export namespace TransformSchema {
    ...
    export function mergeDelta(
        ...
        const toFixed =
            toFixedNum !== undefined
                ? (v: number) => Math.round(v * 100) / 100
                : (v: number) => v
        return {
            position: {
                x: toFixed(
                    newTransformDelta.position.x + oldTransform.position.x,
                ),
                y: toFixed(
                    newTransformDelta.position.y + oldTransform.position.y,
                ),
            },
            size: {
                width: toFixed(
                    newTransformDelta.size.width + oldTransform.size.width,
                ),
                height: toFixed(
                    newTransformDelta.size.height + oldTransform.size.height,
                ),
...
export namespace SizeSchema {
    ...
    export function fixSize(
        currentSize: SizeSchema,
        parentSize: SizeSchema,
    ): number {
        if (
            currentSize.width <= parentSize.width &&
            currentSize.height <= parentSize.height
        )
            return 1
