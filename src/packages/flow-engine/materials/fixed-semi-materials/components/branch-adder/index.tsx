...
export default function BranchAdder(props: PropsType) {
    ...
    const nodeData =
        node.firstChild?.getData<FlowNodeRenderData>(FlowNodeRenderData)
    const playground = usePlayground()
    const flowOperationService = useService(
        FlowOperationService,
    ) as FlowOperationService
    ...
    function addBranch() {
        ...
        setTimeout(() => {
            playground.scrollToView({
                bounds: block.getData<FlowNodeTransformData>(
                    FlowNodeTransformData,
                )!.bounds,
                scrollToCenter: true,
            })
